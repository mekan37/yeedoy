...
as $$
  select case
    ...
    when lower(p_category) ~ 'bar|pub|meyhane|birahane|nargile|kokteyl|gece kul[üu]b|lounge' then 'Mekan'
    when lower(p_category) ~ 'pastane|tatl[ıi]|dondurma|çikolata|f[ıi]r[ıi]n|börek|kek dükkan|şekerleme' then 'Tatlıcı'
    when lower(p_category) ~ 'kahvalt[ıi]|brunch' then 'Kahvaltı'
    when lower(p_category) ~ 'bal[ıi]k|deniz mahs[üu]l|et lokantas[ıi]|et yemek|kebap|dönerci|mangal|barbek[üu]|[ıi]zgara|köfte|biftek|sakatat|tavuk|kelle paça' then 'Balık / Et'
    when lower(p_category) ~ 'kafe|kahve|kafeterya|çay bahçesi|çay evi' then 'Kafe'
    when lower(p_category) ~ 'restoran|restaurant|lokanta|pideci|pizza|hamburger|çorba|mant[ıi]|dür[üu]m|fast food|aşevi|vagon restoran|büfe|tostçu|simitçi|yemek üretic|yemek pazar[ıi]|bistro|sandviç|yeme.{0,4}içme|yiyecek ve içecek' then 'Restoran'
    else null
  end;
$$;

comment on function private._map_google_category_to_business_category(text) is
  'Google Maps kategori string''ini public.businesses.category''nin kapalı taksonomisine (Restoran/Kafe/Kahvaltı/Balık ve Et/Tatlıcı/Mekan) eşler. Eşlenemeyen kategori NULL döner (tahmin yapılmaz). tools/foursquare-ilce-ice-aktar.mjs getFsqCategory() ile aynı önceliklendirme mantığı.';
...
begin
  ...
  with matches as (
    select es.business_id, cat.phone as g_phone, cat.website_url as g_website,
  ...
  update public.businesses b set
    ...
    website_url = case when nullif(trim(b.website_url),'') is null then nullif(trim(best.g_website),'') else b.website_url end,
  ...
  where b.id = best.business_id
    and (
      ...
      (nullif(trim(b.website_url),'') is null and nullif(trim(best.g_website),'') is not null) or
...
revoke execute on function public.enrich_businesses_from_google_catalog_v1() from public, anon, authenticated;
grant execute on function public.enrich_businesses_from_google_catalog_v1() to service_role;

comment on function public.enrich_businesses_from_google_catalog_v1 is
  'business_external_sources(provider=google_maps) ile eşleşmiş işletmelerin YALNIZ boş alanlarını Google katalogdan doldurur. name/lat/lng/is_verified/price_level''a asla dokunmaz. Idempotent: zaten dolu alanlar bir daha güncellenmez.';
...
begin
  ...
  select
    ...
    nullif(trim(cat.website_url),'') as website_url,
    ...
    encode(digest(
      lower(regexp_replace(trim(cat.name), '\s+', ' ', 'g')) || '|' ||
      coalesce(private._map_google_category_to_business_category(cat.category),'') || '|' ||
      lower(trim(cat.city_hint)) || '|' ||
      lower(trim(cat.district_hint)) || '|' ||
      round(cat.lat::numeric,4)::text || '|' ||
      round(cat.lng::numeric,4)::text,
    'sha1'), 'hex') as fp
  ...
  with ins as (
    insert into public.businesses (
      name, category, description, phone, address, city, district, neighborhood,
      lat, lng, website_url, reservation_url, is_active, is_verified, source, source_id, fingerprint
    )
    select
      name, category, description, phone, address, city, district, neighborhood,
      lat, lng, website_url, reservation_url, true, false, 'google_maps', source_key, fp
    from _gm_candidates
  ...
  with conflicted as (
    ...
    and exists (select 1 from public.businesses b2 where b2.fingerprint = c.fp)
  )
  insert into public.business_external_sources (
    business_id, provider, source_key, place_id, cid, data_id,
    source_url, menu_url, plus_code, first_seen_at, last_seen_at
  )
  select b.id, c.provider, c.source_key, c.place_id, c.cid, c.data_id,
         c.source_url, c.menu_url, c.plus_code, c.first_seen_at, c.last_seen_at
  from conflicted c
...
revoke execute on function public.insert_new_businesses_from_google_catalog_v1() from public, anon, authenticated;
grant execute on function public.insert_new_businesses_from_google_catalog_v1() to service_role;

comment on function public.insert_new_businesses_from_google_catalog_v1 is
  'Hiçbir mevcut işletmeyle eşleşmemiş, kategorisi bilinen taksonomiye net eşlenen Google catalog kayıtlarını yeni businesses satırı olarak ekler. source=google_maps, source_id=catalog.source_key, is_verified=false, is_active=true. slug/geog/search_tsv mevcut trigger''larla otomatik. fingerprint çakışırsa mevcut işletmeye sessizce bağlanır, yeni satır açılmaz. Resume-safe: business_external_sources''ta zaten var olan source_key bir daha işlenmez.';
