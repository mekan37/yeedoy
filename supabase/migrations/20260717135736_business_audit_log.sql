...
-- NOTE: Supabase's `public` schema carries default ACLs (set for postgres/
-- supabase_admin) that auto-grant INSERT/UPDATE/DELETE/etc. directly to the
-- `anon` and `authenticated` roles on every newly created table.
-- `REVOKE ALL ... FROM PUBLIC` does NOT undo these — they are direct grants
-- to the roles, not to the PUBLIC pseudo-role — so we must revoke from
-- `anon`/`authenticated` explicitly (see 20260504000002_revoke_anon_grants.sql
-- and 20260520000007_revoke_public_admin_rpc.sql for the established pattern).
REVOKE ALL ON TABLE public.business_audit_log FROM PUBLIC;
REVOKE ALL ON TABLE public.business_audit_log FROM anon;
REVOKE ALL ON TABLE public.business_audit_log FROM authenticated;
...
-- Same default-ACL caveat as above applies to functions: PUBLIC revoke alone
-- leaves anon with a direct EXECUTE grant, so revoke from anon explicitly too.
...
-- No client role should ever call this — only pg_cron (running as the job
-- owner). Revoke from PUBLIC, anon, and authenticated explicitly (default
-- ACLs on the public schema otherwise leave anon/authenticated with EXECUTE).
REVOKE ALL ON FUNCTION public.purge_expired_business_audit_log() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.purge_expired_business_audit_log() FROM anon;
REVOKE ALL ON FUNCTION public.purge_expired_business_audit_log() FROM authenticated;
