drop function if exists public.match_google_catalog_to_businesses_v1(numeric,numeric,numeric,numeric,int);
drop function if exists public.insert_new_businesses_from_google_catalog_v1(int);
drop function if exists public.route_google_business_links_v1(int);
drop function if exists public.insert_business_hours_from_google_catalog_v1(int);
drop function if exists public.find_google_maps_unresolved_candidates_v1(numeric,numeric,int);
drop function if exists public.flag_google_maps_category_unmapped_v1(int);
...
begin
  ...
  with batch_keys as (
    ...
    and (p_city is null or cat.city_hint = p_city)
...
revoke execute on function public.match_google_catalog_to_businesses_v1(numeric,numeric,numeric,numeric,int,text) from public, anon, authenticated;
grant execute on function public.match_google_catalog_to_businesses_v1(numeric,numeric,numeric,numeric,int,text) to service_role;
comment on function public.match_google_catalog_to_businesses_v1(numeric,numeric,numeric,numeric,int,text) is
  'Google Maps catalog kayıtlarını mevcut public.businesses ile geo+isim benzerliğiyle eşleştirir. p_batch_size + p_city ile daraltılabilir (p_city=NULL → tüm şehirler). Idempotent.';
...
revoke execute on function public.insert_new_businesses_from_google_catalog_v1(int,text) from public, anon, authenticated;
grant execute on function public.insert_new_businesses_from_google_catalog_v1(int,text) to service_role;
comment on function public.insert_new_businesses_from_google_catalog_v1(int,text) is
  'Eşleşmemiş, kategorisi eşlenebilen Google catalog kayıtlarını yeni businesses satırı olarak ekler. p_batch_size + p_city ile daraltılabilir. fingerprint UNIQUE + (provider,source_key) UNIQUE ile aynı kayıt iki kez business açamaz.';
...
revoke execute on function public.route_google_business_links_v1(int,text) from public, anon, authenticated;
grant execute on function public.route_google_business_links_v1(int,text) to service_role;
comment on function public.route_google_business_links_v1(int,text) is
  'Google catalog website_url/order_online_url alanlarını domain''e göre doğru businesses kolonuna yönlendirir. p_batch_size + p_city ile daraltılabilir. Idempotent.';
...
revoke execute on function public.insert_business_hours_from_google_catalog_v1(int,text) from public, anon, authenticated;
grant execute on function public.insert_business_hours_from_google_catalog_v1(int,text) to service_role;
comment on function public.insert_business_hours_from_google_catalog_v1(int,text) is
  'business_external_sources(provider=google_maps) ile ilişkili işletmeler için Google opening_hours JSONB''ını business_weekly_hours''a normalize eder. p_batch_size + p_city ile daraltılabilir. ON CONFLICT DO NOTHING: mevcut gün satırı asla overwrite edilmez.';
...
revoke execute on function public.find_google_maps_unresolved_candidates_v1(numeric,numeric,int,text) from public, anon, authenticated;
grant execute on function public.find_google_maps_unresolved_candidates_v1(numeric,numeric,int,text) to service_role;
comment on function public.find_google_maps_unresolved_candidates_v1(numeric,numeric,int,text) is
  'match_google_catalog_to_businesses_v1''in tier eşiklerini geçemeyen ama p_min_sim üzerinde kalan en-iyi-aday eşleşmeleri unresolved_candidates''a yazar. p_batch_size + p_city ile daraltılabilir. Idempotent.';
...
revoke execute on function public.flag_google_maps_category_unmapped_v1(int,text) from public, anon, authenticated;
grant execute on function public.flag_google_maps_category_unmapped_v1(int,text) to service_role;
comment on function public.flag_google_maps_category_unmapped_v1(int,text) is
  'Kategorisi eşlenemeyen catalog satırlarını unresolved_candidates''a taşır. p_batch_size + p_city ile daraltılabilir. Idempotent.';
