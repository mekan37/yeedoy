-- Bug: team_seat_count sadece business_team_memberships satırlarını sayıyordu.
-- owner_claims (mülkiyet) ile business_team_memberships (davet edilen ekip üyeleri) ayrık
-- tablolar; işletmeyi sahiplenen kullanıcı kendine otomatik bir üyelik satırı almıyor.
-- Sonuç: free tier (limit=1) sahibi used=0 görüyor, "1 koltuk = sadece sahip" tasarım
-- niyetine rağmen hâlâ 1 kişi davet edebiliyordu. Sahibin kendi koltuğu için +1 ekleniyor.
...
COMMENT ON FUNCTION public._check_plan_limit_v1 IS
  'Plan limiti kontrolü. menu_item_count/team_seat_count/branch_count anlık satır sayımı, diğerleri plan_feature_usage aylık sayacı. team_seat_count sahibin kendi koltuğu için +1 içerir. Aşılmışsa P0003 plan_limit_exceeded fırlatır.';
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'used', CASE
      ...
      WHEN pf.feature_key = 'team_seat_count' THEN (
        SELECT 1 + count(*) FROM public.business_team_memberships btm
        WHERE btm.revoked_at IS NULL
          AND (
