-- Sahip Paneli Güvenlik Denetimi P1: upsert_team_member_v1'de rol
-- yükseltme koruması yoktu. team_manage izni (rank>=400, yani manager de
-- dahil) yeterli görülüyordu, 'owner' atanabilir roller arasındaydı ve
-- çağıranın kendi üyeliğini hedef almasını engelleyen hiçbir kontrol
-- yoktu — bir manager tek bir doğrudan RPC çağrısıyla
-- (p_email=kendi_email, p_role='owner') kendini işletmenin sahibi
-- yapabiliyordu.
--
-- Üç guard eklendi:
--   1) 'owner' artık hiçbir zaman atanabilir değil (admin dahil) —
--      sahiplik yalnızca owner_claims onay akışından geçmeli.
--   2) Atanan rolün rütbesi çağıranın kendi rütbesinden düşük olmalı
--      (admin hariç) — eşit/yüksek rütbe ataması reddedilir.
--   3) Çağıran, kendi üyeliğini bu RPC üzerinden düzenleyemez (admin
--      hariç).
...
BEGIN
  ...
  if not public.is_admin() and v_user_id is not null and v_user_id = auth.uid() then
    return jsonb_build_object('ok', false, 'code', 'cannot_edit_self');
  end if;
