...
import '../../../core/i18n/currency_formatter.dart';
            ...
            return _maxWidth(
              ...
              ListView(
                ...
                children: [
                  AppHeroHeader(
                    title: item.name,
                    subtitle: _formatPrice(item.price),
                        ...
                        if (context.mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                submission.queued
                                    ? 'Baglanti zayif. Dogrulama kuyruga alindi, internet gelince otomatik gonderilecek.'
                                    : submission.autoApproved
                                    ? 'Fiyat otomatik onaylandı ve menü güncellendi.'
                                    : (submission.pendingCount > 1
                                          ? 'Önerin alındı. Bu ürün için ${submission.pendingCount} öneri sırada.'
                    ...
                    onUpload: () async {
                      ...
                      try {
                        ...
                        if (quality != null &&
                            ...
                            context.mounted) {
                          final warnings = [
                            if (quality.isDark) 'karanlık',
                            if (quality.isBlurry) 'bulanık',
                          ].join(' / ');
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
        ...
        child: Consumer(
          builder: (context, ref, _) {
            ...
            return progressAsync.when(
              ...
              error: (err, stack) => const _ThanksContent(
                levelLabel: 'Katkı Seviyesi',
                levelValue: 'Seviye 1',
                progress: 0.2,
                scoreText: 'Puanın güncellendi',
              ),
              data: (progress) {
...
class _PriceHistoryRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final current = _formatPriceFromCents(item.priceCents) ?? '-';
    final previous = _formatPriceFromCents(item.oldPriceCents);
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(
                previous == null
                    ? '$current â‚º ${item.source}'
                    : '$previous â‚º $current â‚º ${item.source}',
                style: const TextStyle(
                  fontWeight: FontWeight.w800,
                  color: AppColors.textStrong,
                ),
              ...
              Text(
                '${_relativeTime(context, item.createdAt)} • ${_fmtDate(item.createdAt)}${deltaText == null ? '' : ' • $deltaText'}',
                style: const TextStyle(color: AppColors.muted, fontSize: 12),
              ),
            ],
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return AppCard(
      child: statusAsync.when(
        ...
        data: (status) {
          final priceText =
              _formatPriceFromCents(status.priceCents) ??
...
String _formatPrice(double? price) {
  return formatCurrency(price, currency: 'TRY', languageCode: 'tr');
}
