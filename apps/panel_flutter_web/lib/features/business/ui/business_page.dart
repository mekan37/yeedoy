...
import '../../../src/features/business/business_detail_controller.dart';
...
final _businessProvider = FutureProvider.family<Business, String>((ref, id) {
  return ref.watch(discoveryRepoProvider).getBusiness(id);
});
...
final _businessTrustProvider =
    FutureProvider.family<_BusinessTrustSnapshot, String>((
      ...
      final verifiedRows = await client
          .from('menu_item_price_suggestions')
          .select('created_at,created_by,quality_confidence')
          .eq('business_id', businessId)
          .inFilter('status', ['approved', 'accepted', 'handled', 'verified'])
          .gte(
            'created_at',
      ...
      final rows = (verifiedRows as List).whereType<Map>().toList(
        growable: false,
      );
...
class _BusinessSectionsScroll extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final menuItems = trendingAsync.maybeWhen(
      data: (items) => items.take(8).toList(growable: false),
      orElse: () => const <BusinessTrendingItem>[],
    );
    final heroImage = _heroImageForCategory(business.category);
              ...
              children: [
                ...
                trustAsync.when(
                  ...
                  error: (error, _) => AppEmptyState(
                    ...
                    ctaLabel: MicrocopyStyleGuide.retry,
                    onCta: () =>
...
class _MenuCategoriesRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final t = AppLocalizations.of(context);
    final labels = [t.tabAllItems, t.tabStarters, t.tabSteaks, t.tabSides];
        ...
        children: [
          for (var i = 0; i < labels.length; i++) ...[
            Container(
              ...
              decoration: BoxDecoration(
                color: i == 0 ? AppColors.primary : AppColors.cardAlt,
              ...
              child: Text(
                labels[i],
                style: TextStyle(
                  color: i == 0 ? AppColors.onPrimary : AppColors.textStrong,
                  fontWeight: FontWeight.w800,
                ),
              ),
            ),
...
class _MenuItemsSection extends StatelessWidget {
  ...
  final List<BusinessTrendingItem> items;
        ...
        children: [
          ...
          for (var i = 0; i < items.length; i++) ...[
            _MenuItemRow(
              ...
              showWeeklyChange: i % 3 == 1,
              description:
                  '$businessCategory ${AppLocalizations.of(context).featuredCuisineSuffix}',
            ),
...
class _MenuItemRow extends StatelessWidget {
  const _MenuItemRow({
    required this.item,
    required this.imageAsset,
    required this.showWeeklyChange,
    required this.description,
  });

  final BusinessTrendingItem item;
  final String imageAsset;
  final bool showWeeklyChange;
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Text(
                    _formatPriceWithCurrency(context, item.priceCents, '?'),
                    style: const TextStyle(
                      fontWeight: FontWeight.w900,
                      color: AppColors.textStrong,
                      fontSize: 20,
              ...
              Row(
                children: [
                  ...
                  if (showWeeklyChange) ...[
                    const SizedBox(width: 8),
                    Text(
                      AppLocalizations.of(context).weeklyPriceChange,
                      style: const TextStyle(
                        color: AppColors.muted,
                        fontWeight: FontWeight.w700,
                        fontSize: 12,
            ...
            data: (menus) {
              if (menus.isEmpty) {
                return AppEmptyState(
                  ...
                  ctaLabel: MicrocopyStyleGuide.addFirstMenu,
                  onCta: () => _openReportSheet(context, businessId),
                );
              }
              return Column(
