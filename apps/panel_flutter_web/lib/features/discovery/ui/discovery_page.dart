...
import '../../../src/ui/sections/top_businesses_strip.dart';
...
class _RecommendedTabState extends ConsumerState<_RecommendedTab>
  ...
  Widget build(BuildContext context) {
    ...
    final canSeeOwnerTools = appRoleAsync.maybeWhen(
    ...
    final areaLabel = hasNeighborhood
        ...
        : '$district bÃ¶lgesinde';
    final rankLabelPrefix = hasNeighborhood ? 'Mahallende' : 'Ä°lÃ§ende';
  ...
  Future<void> _trackDiscoveryClick(
    ...
    await _logDiscoveryClick(ref, businessId: businessId, source: source);
  }
  ...
  Widget _buildPremiumDiscoveryLayout({
    ...
    final nearbyItems = items.take(10).toList();
  ...
  String _categoryImageFor(String category, int index) {
    ...
    for (final cfg in discoveryHomeCategories) {
      if (normalized.contains(cfg.searchTerm.toLowerCase()) ||
          normalized.contains(cfg.title.toLowerCase())) {
...
class _V5RevenueHubCard extends ConsumerWidget {
  const _V5RevenueHubCard();
    ...
    return AppCard(
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.payments_outlined, color: AppColors.textStrong),
              SizedBox(width: 8),
              Expanded(
                child: Text(
                  t.businessTools,
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              _V5ItemChip(t.businessPackage, ready: true),
              _V5ItemChip(
                t.sponsoredLabelChip,
                ready: guardrails.requireSponsoredLabel,
              ),
              _V5ItemChip(t.redirectToReservation, ready: true),
              _V5ItemChip(t.priceAlerts, ready: true),
              _V5ItemChip(t.corporateIntegration, ready: false),
              _V5ItemChip(t.detailedReports, ready: true),
              _V5ItemChip(t.qrTools, ready: true),
              _V5ItemChip(t.unlockNewFeatures, ready: v3Discovery),
            ],
          ),
          const SizedBox(height: 10),
          Wrap(
            ...
            children: [
              OutlinedButton.icon(
                onPressed: () => context.go('/owner/perks'),
                icon: const Icon(Icons.workspace_premium_outlined, size: 18),
                label: Text(t.businessPackage),
              ),
              OutlinedButton.icon(
                onPressed: () => context.go('/admin/sponsorships'),
                icon: const Icon(Icons.campaign_outlined, size: 18),
                label: Text(t.sponsored),
              ),
              OutlinedButton.icon(
                onPressed: () => context.go('/owner/businesses'),
                icon: const Icon(Icons.storefront_outlined, size: 18),
                label: Text(t.branchManagement),
              ),
              OutlinedButton.icon(
                onPressed: () => context.go('/admin/b2b-exports'),
                icon: const Icon(Icons.dataset_outlined, size: 18),
                label: Text(t.detailedReports),
              ),
              OutlinedButton.icon(
                onPressed: () => context.go('/owner/menus'),
                icon: const Icon(Icons.qr_code_2_outlined, size: 18),
                label: Text(t.menuWithQr),
              ),
              FilledButton.tonalIcon(
                onPressed: () => context.go('/admin/dev-tools'),
                icon: const Icon(Icons.tune_outlined, size: 18),
                label: Text(t.newFeatures),
              ),
            ],
          ),
        ],
...
class _V5ItemChip extends StatelessWidget {
  const _V5ItemChip(this.label, {required this.ready});
  ...
  Widget build(BuildContext context) {
    ...
    final t = AppLocalizations.of(context);
    final stateText = ready ? t.ready : t.plan;
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
      decoration: BoxDecoration(
        color: color.withValues(alpha: 0.12),
...
_StatusBadgeConfig _statusBadge(BuildContext context, String? status) {
  ...
  switch (status) {
    case 'verified':
      return _StatusBadgeConfig(t.statusVerifiedShort, AppColors.success);
    case 'mixed':
      return _StatusBadgeConfig(t.statusMixedShort, AppColors.warning);
    case 'outdated':
      return _StatusBadgeConfig(t.statusOutdatedShort, AppColors.danger);
    default:
      return _StatusBadgeConfig(t.statusUnknownShort, AppColors.muted);
  }
}

class _StatusBadgeConfig {
  const _StatusBadgeConfig(this.label, this.color);
  final String label;
  final Color color;
}
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 8),
          for (final category in discoveryFilterLabels) ...[
            CategoryChip(
              label: category,
                    ...
                    for (final b in items) ...[
                      Container(
                        ...
                        child: BusinessTile(
                          ...
                          category: b.category,
                          subtitle:
                              '${t.sponsored} Â· ${b.district ?? ''} ${b.city ?? ''}',
                  ...
                  Padding(
                    ...
                    child: BusinessTile(
                      ...
                      socialProof: [
                        AppLocalizations.of(
                          context,
                        ).rankedAt(rankLabelPrefix, entry.key + 1),
