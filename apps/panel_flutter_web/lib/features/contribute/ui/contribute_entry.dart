...
import '../../../features/discovery/data/discovery_repo.dart';
import '../../../src/ui/design_system.dart';
import '../data/temp_uploads_repository.dart';
...
class _ContributeFabState extends ConsumerState<ContributeFab> {
  ...
  Future<void> _captureAndUpload({
    ...
    if (businessId == null || businessId.trim().isEmpty) {
      _toast('Bu katkÃ„Â± iÃƒÂ§in ÃƒÂ¶nce bir iÃ…Å¸letme sayfasÃ„Â± aÃƒÂ§.');
      return;
    }
    ...
    try {
      ...
      if (count <= 1) {
        _toast('GÃƒÂ¶nderildi - kontrol sonrasi menuye eklenecek.');
      } else {
        _toast(
          '$count sayfa gÃƒÂ¶nderildi - kontrol sonrasÃ„Â± menÃƒÂ¼ye eklenecek.',
        );
      }
    } catch (_) {
      if (!mounted) return;
      Navigator.of(context, rootNavigator: true).pop();
      _toast('GÃƒÂ¶nderim baÃ…Å¸arÃ„Â±sÃ„Â±z. LÃƒÂ¼tfen tekrar dene.');
    }
  }
  ...
  Future<void> _captureQrAndSmartRoute() async {
    ...
    _showProgress('QR ÃƒÂ§ÃƒÂ¶zÃƒÂ¼lÃƒÂ¼yor...');
    try {
      ...
      if (decoded == null || decoded.trim().isEmpty) {
        _toast('QR okunamadÃ„Â±. GÃƒÂ¶rsel inceleme iÃƒÂ§in gÃƒÂ¶nderiliyor.');
      ...
      if (targetRoute != null) {
        _toast('QR doÃ„Å¸rulandÃ„Â±. YÃƒÂ¶nlendiriliyorsun.');
        if (!mounted) return;
      ...
      if (!mounted) return;
      Navigator.of(context, rootNavigator: true).pop();
      _toast('QR iÃ…Å¸lenemedi. LÃƒÂ¼tfen tekrar dene.');
    }
  }
  ...
  Future<void> _uploadQrForReview(XFile file, {String? scannedText}) async {
    final businessId = widget.businessId;
    if (businessId == null || businessId.trim().isEmpty) {
      _toast('DÃ„Â±Ã…Å¸ QR kodu iÃƒÂ§in iÃ…Å¸letme sayfasÃ„Â±nda KatkÃ„Â± yap kullan.');
      return;
    }
    _showProgress('Ã„Â°nceleme iÃƒÂ§in gÃƒÂ¶nderiliyor...');
    try {
      final userId = ref.read(userProvider)?.id;
      await ref
          .read(tempUploadsRepositoryProvider)
      ...
      _toast(
        scannedText == null
            ? 'QR gÃƒÂ¶rseli gÃƒÂ¶nderildi. Ã„Â°nceleme sonrasÃ„Â± iÃ…Å¸leme alÃ„Â±nacak.'
            : 'DÃ„Â±Ã…Å¸ link admin incelemesine gÃƒÂ¶nderildi.',
      );
    } catch (_) {
      if (!mounted) return;
      Navigator.of(context, rootNavigator: true).pop();
  ...
  void _openVerifyFlow() {
    final businessId = widget.businessId;
    if (businessId == null || businessId.trim().isEmpty) {
      _toast('Fiyat doÃ„Å¸rulama iÃƒÂ§in ÃƒÂ¶nce bir iÃ…Å¸letme seÃƒÂ§.');
      return;
    }
    context.go('/b/$businessId');
    _toast('MenÃƒÂ¼den tek ÃƒÂ¼rÃƒÂ¼nÃƒÂ¼ seÃƒÂ§ip fiyatÃ„Â± doÃ„Å¸rulayabilirsin.');
  }
