...
import '../../../src/data/repositories/collection_social_repository.dart';
import '../../../src/data/repositories/collection_share_repository.dart';
...
class _FavoritesPageState extends ConsumerState<FavoritesPage> {
          ...
          children: [
            AppHeroHeader(
              ...
              subtitle: _isSharedMode
                  ? 'Paylaşılan koleksiyon: ${_sharedName ?? widget.sharedCollectionName ?? 'Koleksiyon'}'
  ...
  Widget _nearbyBanner() {
    final text = _locLoading
        ? 'Yakınındakiler modu için konum alınıyor...'
        : (_pos == null
              ? 'Konum alınamadığı için varsayılan sıralama gösteriliyor.'
              : 'Yakınındakiler modunda mesafeye göre sıralandı.');
    return Padding(
      padding: const EdgeInsets.only(bottom: 10),
      child: AppCard(
        child: Row(
        ...
        children: [
          Row(
            children: [
              ...
              IconButton(
                tooltip: 'Koleksiyon oluştur',
                onPressed: _createCollection,
                icon: const Icon(Icons.create_new_folder_outlined),
              ),
              if (_selectedCollectionId.isNotEmpty) ...[
                IconButton(
                  tooltip: 'Koleksiyonu paylaş',
      ...
      return AppCard(
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'İçerik üretici modu: Yayınlamak için bir koleksiyon seç.',
    ...
    return AppCard(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            'Koleksiyonunu yayınla, takipçi kazan. Reklam içeriği varsa etiketi zorunlu.',
            style: const TextStyle(color: AppColors.muted),
          ),
          ...
          SwitchListTile(
            ...
            subtitle: Text(
              col.isPublic
                  ? 'Profilinde ve paylaşım linklerinde görünür.'
                  : 'Sadece sen görürsün.',
            ),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              _InfoChip(text: 'Takipçi $followers', color: AppColors.header),
              const SizedBox(width: 8),
              _InfoChip(
                text: 'Etkileşim $engagement',
  ...
  Future<void> _createCollection() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          controller: ctrl,
          decoration: const InputDecoration(hintText: 'Örn: Gece döneri'),
        ),
        actions: [
          TextButton(
  ...
  Future<void> _deleteSelectedCollection() async {
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Koleksiyon silinsin mi?'),
                  ...
                  Row(
                    children: [
                      OutlinedButton(
                        ...
                        child: const Text('Yeni koleksiyon'),
                      ),
                      const Spacer(),
                      FilledButton(
                        onPressed: () async {
  ...
  String _sharedCollectionKey() {
    ...
    final name = _sharedName ?? widget.sharedCollectionName ?? 'Koleksiyon';
  ...
  Future<bool?> _askAdDisclosure() {
    return showDialog<bool>(
      ...
      builder: (ctx) {
        return AlertDialog(
          title: const Text('Reklam bildirimi'),
          content: const Text(
            'Bu koleksiyon bir iş birliği içeriyorsa "Reklam" olarak işaretlemek zorunlu.',
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(ctx, false),
              child: const Text('Organik'),
            ),
            FilledButton(
              onPressed: () => Navigator.pop(ctx, true),
...
class _CreatorBadgesRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Wrap(
      ...
      children: [
        _InfoChip(
          text: isSponsored ? 'Reklam' : 'Organik',
          color: isSponsored ? AppColors.warning : AppColors.primary,
        ),
        _InfoChip(text: 'Takipçi $followers', color: AppColors.header),
        _InfoChip(text: 'Etkileşim $engagement', color: AppColors.success),
      ],
    );
  }
}
...
class _FavoriteBusinessCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (business.isOpenNow == true) {
      chips.add(_InfoChip(text: 'Şimdi açık', color: AppColors.success));
    }
    if ((business.recentPriceVerifiedCount ?? 0) > 0) {
      chips.add(
        _InfoChip(text: 'Son fiyat doğrulandı', color: AppColors.primary),
      );
    }
        ...
        children: [
          Row(
            children: [
              ...
              IconButton(
                tooltip: isFavorited ? 'Favoriden çıkar' : 'Favoriye ekle',
