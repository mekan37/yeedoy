import 'package:flutter_riverpod/flutter_riverpod.dart';

import '../data/admin_price_suggestions_repository.dart';
import 'admin_models.dart';

class AdminPriceSuggestionsState {
  const AdminPriceSuggestionsState({
    required this.items,
    required this.isLoading,
    required this.isLoadingMore,
    required this.hasMore,
    required this.error,
    required this.statusFilter,
    required this.assignedFilter,
    required this.slaOnly,
  });

  final List<AdminMenuPriceSuggestionItem> items;
  final bool isLoading;
  final bool isLoadingMore;
  final bool hasMore;
  final Object? error;
  final String statusFilter;
  final String assignedFilter;
  final bool slaOnly;

  factory AdminPriceSuggestionsState.initial() =>
      const AdminPriceSuggestionsState(
        items: [],
        isLoading: false,
        isLoadingMore: false,
        hasMore: true,
        error: null,
        statusFilter: 'pending',
        assignedFilter: '',
        slaOnly: false,
      );

  AdminPriceSuggestionsState copyWith({
    List<AdminMenuPriceSuggestionItem>? items,
    bool? isLoading,
    bool? isLoadingMore,
    bool? hasMore,
    Object? error,
    String? statusFilter,
    String? assignedFilter,
    bool? slaOnly,
  }) {
    return AdminPriceSuggestionsState(
      items: items ?? this.items,
      isLoading: isLoading ?? this.isLoading,
      isLoadingMore: isLoadingMore ?? this.isLoadingMore,
      hasMore: hasMore ?? this.hasMore,
      error: error,
      statusFilter: statusFilter ?? this.statusFilter,
      assignedFilter: assignedFilter ?? this.assignedFilter,
      slaOnly: slaOnly ?? this.slaOnly,
    );
  }
}

final adminPriceSuggestionsControllerProvider =
    NotifierProvider<
      AdminPriceSuggestionsController,
      AdminPriceSuggestionsState
    >(AdminPriceSuggestionsController.new);

class AdminPriceSuggestionsController
    extends Notifier<AdminPriceSuggestionsState> {
  static const int pageSize = 50;
  int _requestId = 0;

  @override
  AdminPriceSuggestionsState build() {
    Future.microtask(loadInitial);
    return AdminPriceSuggestionsState.initial();
  }

  Future<bool> loadInitial({bool force = false}) async {
    if (state.isLoading && !force) return false;
    final reqId = ++_requestId;
    state = state.copyWith(isLoading: true, isLoadingMore: false, error: null);
    try {
      final repo = ref.read(adminPriceSuggestionsRepositoryProvider);
      final items = await repo.listSuggestions(
        status: state.statusFilter,
        slaOnly: state.slaOnly,
        assigned: state.assignedFilter,
        limit: pageSize,
        offset: 0,
      );
      if (reqId != _requestId) return false;
      state = state.copyWith(
        items: items,
        isLoading: false,
        hasMore: items.length == pageSize,
      );
      return true;
    } catch (e) {
      if (reqId != _requestId) return false;
      state = state.copyWith(isLoading: false, error: e);
      return false;
    }
  }

  Future<void> loadMore() async {
    if (state.isLoading || state.isLoadingMore || !state.hasMore) return;
    final reqId = _requestId;
    state = state.copyWith(isLoadingMore: true, error: null);
    try {
      final repo = ref.read(adminPriceSuggestionsRepositoryProvider);
      final items = await repo.listSuggestions(
        status: state.statusFilter,
        slaOnly: state.slaOnly,
        assigned: state.assignedFilter,
        limit: pageSize,
        offset: state.items.length,
      );
      if (reqId != _requestId) return;
      state = state.copyWith(
        items: [...state.items, ...items],
        isLoadingMore: false,
        hasMore: items.length == pageSize,
      );
    } catch (e) {
      if (reqId != _requestId) return;
      state = state.copyWith(isLoadingMore: false, error: e);
    }
  }

  Future<bool> refresh({bool force = false}) async {
    _requestId++;
    state = state.copyWith(
      isLoading: true,
      isLoadingMore: false,
      hasMore: true,
      items: [],
      error: null,
    );
    return loadInitial(force: true);
  }

  void setStatusFilter(String status) {
    state = state.copyWith(statusFilter: status);
    loadInitial();
  }

  void setAssignedFilter(String assigned) {
    state = state.copyWith(assignedFilter: assigned);
    loadInitial();
  }

  void setSlaOnly(bool value) {
    state = state.copyWith(slaOnly: value);
    loadInitial();
  }

  Future<void> approve(String suggestionId) async {
    final prev = state.items;
    final updated = prev.map((c) {
      if (c.id != suggestionId) return c;
      return _copyItem(c, status: 'approved');
    }).toList();
    final next = _maybeRemoveAfterStatus(updated, suggestionId, 'approved');
    state = state.copyWith(items: next);
    try {
      await ref
          .read(adminPriceSuggestionsRepositoryProvider)
          .approve(suggestionId);
      await _refreshPreservePaging();
    } catch (e) {
      state = state.copyWith(items: prev);
      rethrow;
    }
  }

  Future<void> reject({
    required String suggestionId,
    required String note,
  }) async {
    final prev = state.items;
    final updated = prev.map((c) {
      if (c.id != suggestionId) return c;
      return _copyItem(c, status: 'rejected');
    }).toList();
    final next = _maybeRemoveAfterStatus(updated, suggestionId, 'rejected');
    state = state.copyWith(items: next);
    try {
      await ref
          .read(adminPriceSuggestionsRepositoryProvider)
          .reject(suggestionId: suggestionId, note: note);
      await _refreshPreservePaging();
    } catch (e) {
      state = state.copyWith(items: prev);
      rethrow;
    }
  }

  Future<void> _refreshPreservePaging() async {
    if (state.isLoading) return;
    final reqId = ++_requestId;
    final limit = state.items.isEmpty ? pageSize : state.items.length;
    state = state.copyWith(isLoading: true, isLoadingMore: false, error: null);
    try {
      final repo = ref.read(adminPriceSuggestionsRepositoryProvider);
      final items = await repo.listSuggestions(
        status: state.statusFilter,
        slaOnly: state.slaOnly,
        assigned: state.assignedFilter,
        limit: limit,
        offset: 0,
      );
      if (reqId != _requestId) return;
      state = state.copyWith(
        items: items,
        isLoading: false,
        hasMore: items.length == limit,
      );
    } catch (e) {
      if (reqId != _requestId) return;
      state = state.copyWith(isLoading: false, error: e);
    }
  }

  List<AdminMenuPriceSuggestionItem> _maybeRemoveAfterStatus(
    List<AdminMenuPriceSuggestionItem> items,
    String suggestionId,
    String status,
  ) {
    if (state.statusFilter.isEmpty) return items;
    if (status == state.statusFilter) return items;
    return items.where((c) => c.id != suggestionId).toList();
  }
}

AdminMenuPriceSuggestionItem _copyItem(
  AdminMenuPriceSuggestionItem c, {
  String? status,
}) {
  return AdminMenuPriceSuggestionItem(
    id: c.id,
    status: status ?? c.status,
    createdAt: c.createdAt,
    businessId: c.businessId,
    businessName: c.businessName,
    city: c.city,
    district: c.district,
    menuItemId: c.menuItemId,
    menuItemName: c.menuItemName,
    currentPriceCents: c.currentPriceCents,
    suggestedPriceCents: c.suggestedPriceCents,
    currency: c.currency,
    createdBy: c.createdBy,
    assignedTo: c.assignedTo,
    assignedAt: c.assignedAt,
    ageHours: c.ageHours,
    slaBreached: c.slaBreached,
    qualityConfidence: c.qualityConfidence,
    anomalyScore: c.anomalyScore,
    anomalyFlags: c.anomalyFlags,
    conflictState: c.conflictState,
    conflictVariants24h: c.conflictVariants24h,
  );
}
