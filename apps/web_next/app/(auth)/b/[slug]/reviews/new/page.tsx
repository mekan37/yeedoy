      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            ...
            className="text-xl transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/40 rounded"
          >
...
export default function ReviewNewPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!user) { router.push(`/login?redirect=/b/${slug}/reviews/new`); return; }
      setUserId(user.id);
  ...
  return (
    <main className="min-h-screen bg-bg">
      <div className="mx-auto max-w-lg px-4 py-12">
        ...
        <h1 className="mb-1 text-2xl font-[900] text-textStrong">Yorum Yaz</h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="rounded-[20px] border border-border bg-cardAlt p-5 shadow-yd1">
            <p className="mb-3 text-sm font-[900] text-textStrong">Genel Puan <span className="text-danger">*</span></p>
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((star) => (
                <button key={star} type="button" onClick={() => setRating(star)}
                  aria-label={`${star} yıldız`}
                  className="focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-primary/40 rounded cursor-pointer">
          ...
          {/* Criteria ratings */}
          <div className="rounded-[20px] border border-border bg-cardAlt p-5 shadow-yd1">
            <p className="mb-4 text-sm font-[900] text-textStrong">Detaylı Değerlendirme <span className="text-[11px] font-[400] text-muted">(isteğe bağlı)</span></p>
            <div className="flex flex-col gap-3">
              {CRITERIA.map(({ key, label }) => (
                <StarRow key={key} value={criteria[key]} onChange={(v) => setCriterion(key, v)} label={label} />
              ))}
            </div>
          </div>
          ...
          {/* Title */}
          <div>
            <label className="mb-2 block text-sm font-[700] text-textStrong">Başlık <span className="text-[11px] font-[400] text-muted">(isteğe bağlı)</span></label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={80}
              placeholder="Kısa bir başlık ekleyin…"
              className="w-full rounded-2xl border border-border bg-bg px-4 py-2.5 text-sm text-textStrong placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-primary/30" />
          </div>

          {/* Content */}
          <div>
            <label className="mb-2 block text-sm font-[700] text-textStrong">Yorumunuz <span className="text-danger">*</span></label>
          ...
          {error && (
            <div className="rounded-xl border border-danger/20 bg-danger/[0.08] px-4 py-3 text-sm font-[700] text-danger">
              {error}
            </div>
          )}

          <button type="submit" disabled={loading || !bizId}
            className="min-h-[52px] w-full rounded-2xl text-base font-[800] text-white transition-all hover:-translate-y-px hover:brightness-105 active:scale-[0.97] disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
