...
export async function generatePublicMenuMetadata(input: {
  ...
  const title = [itemName, categoryName, businessName, 'QR Menu'].filter(Boolean).join(' | ');
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      images: data.media.coverUrl ? [data.media.coverUrl] : [`/api/og?title=${encodeURIComponent(businessName)}`],
    },
    twitter: {
