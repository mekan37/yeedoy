...
`apps/web_next` production release kapsaminda yalnizca su yuzeyleri sunar:
...
Owner ve admin CRUD islemleri `apps/panel_flutter_web` icinde kalir. `web_next` panelden link veya session handoff ile baslatilan dagitim ve sunum katmanidir.

## Kapatilan Kritik Sorunlar

### 1. Owner handoff payload dogrulamasi duzeltildi

- `app/auth/panel-handoff/route.ts` icindeki `refresh_token` uzunluk varsayimi kaldirildi.
...
- `app/api/track/route.ts` artik RPC sonucu `{ ok: false }` ise sessiz `200` donmez.
...
|---|---|---:|---:|---:|---:|
| minimal | `/m/6f3f0372-65a4-40ef-b915-da05908d98c7?lang=tr&theme=minimal` | 99 | 100 | 96 | 90 |
| photo-heavy | `/m/6f3f0372-65a4-40ef-b915-da05908d98c7?lang=tr&theme=photo-heavy` | 99 | 100 | 96 | 90 |
| dark-modern | `/m/6f3f0372-65a4-40ef-b915-da05908d98c7?lang=tr&theme=dark-modern` | 99 | 100 | 96 | 90 |
| qr bold | `/qr/6f3f0372-65a4-40ef-b915-da05908d98c7?lang=tr&theme=bold` | 97 | 100 | 96 | 100 |
...
- Max upload: `5MB`
- Path izolasyonu:
  - `businesses/{businessId}/branding/{type}/{uuid}.{ext}`
- Yetki:
  - `can_manage_business_v1(business_id)` zorunlu
- Storage cache suresi:
  - `31536000`
- Public render cache bust:
  - `updated_at` tabanli `?v=` param'i
...
- QR Studio Lighthouse accessibility notu:
  - `target-size` nedeniyle `91`
