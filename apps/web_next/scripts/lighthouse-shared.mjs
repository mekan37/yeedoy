...
export function writePerfMarkdown() {
  ...
  const loginReport = readSummary('login');
  const qrAuthReport = readSummary('qr-auth');
...
export async function getSessionCookiesViaPanelHandoff({ baseUrl, businessId, lang, theme, accessToken, refreshToken }) {
  ...
  const response = await fetch(`${baseUrl}/auth/panel-handoff`, {
    method: 'POST',
    body: formData,
    redirect: 'manual',
  });
  ...
  if (response.status !== 200) {
    throw new Error(`Panel handoff failed: ${response.status}`);
  }

  const rawCookies = response.headers.getSetCookie?.() ?? [];
  if (!rawCookies.length) {
    throw new Error('Panel handoff did not return auth cookies');
  }
...
${loginReport ? renderSingleReportTable(loginReport) : '_Henuz olculmedi._'}
...
- Login Gate:
  ${loginReport ? `${loginReport.targetSize.items.length} aktif node, hedef skor ${loginReport.scores.accessibility}` : 'olcum yok'}
