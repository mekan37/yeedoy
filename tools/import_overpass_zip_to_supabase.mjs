...
const ZIP_PATH = process.env.ZIP_PATH || './out_overpass.zip';
...
function parseCityDistrictFromEntry(entryName) {
  // Example: out_overpass/ankara__yeni-mahalle__6_1723.json
  // city__district__...
...
async function main() {
  ...
  const entries = zip.getEntries()
    .filter(e => !e.isDirectory && e.entryName.endsWith('.json') && e.entryName.includes('out_overpass/'));
