...
import 'package:yeedoy/core/utils/greeting_utils.dart';
  ...
  group('ProfilePage redesign (guest)', () {
    testWidgets(
      ...
      (tester) async {
        ...
        // Selamlama artık statik değil, timeBasedGreeting() ile saate göre
        // değişiyor (Günaydın/Merhaba/İyi Akşamlar/İyi Geceler) — B65
        // test-rot düzeltmesi: sabit "Merhaba! 👋" yerine gerçek fonksiyon
        // çağrılıyor.
        expect(find.text(timeBasedGreeting()), findsOneWidget);
        ...
        // Account list: section title + rows (Diyet Profilim, Sadakat
        // Kartlarım, Bildirim Tercihleri, Yardım ve Destek) — "Adreslerim"
        // satırı kaldırılmış, artık yok; "Hesap Güvenliği" satırı da yok
        // (B65 test-rot düzeltmesi: mevcut tasarıma göre güncellendi).
        expect(find.text(t.profileAccountSectionTitle), findsOneWidget);
        expect(find.text(t.profileMenuDietTitle), findsOneWidget);
        expect(find.text(t.profileMenuLoyaltyTitle), findsOneWidget);
        expect(find.text(t.profileMenuNotificationsTitle), findsOneWidget);
        expect(find.text(t.profileMenuHelpTitle), findsOneWidget);
        ...
        // device location, and the account list no longer has an
        // "Adreslerim" row (removed — see above), so this icon shouldn't
        // appear anywhere on the page (B65 test-rot düzeltmesi).
