...
void main() {
  ...
  group('OfflineMutationQueueStore', () {
    test(
      'migrates legacy queue payloads into unified local db bucket',
      () async {
        SharedPreferences.setMockInitialValues(<String, Object>{
    ...
    test(
      'markRetry stores retry metadata and network backoff timestamp',
      () async {
    ...
    test('uses slower backoff for auth and rate limit errors', () async {
      ...
      final updatedRate = updated.firstWhere(
        (item) => item.id == 'verify_rate',
      );
    ...
    test('dropForUserMismatch removes only items owned by a different user, '
        'keeps unowned (legacy) items', () async {
      final seededAt = DateTime.utc(2026, 3, 7, 12);
      await OfflineMutationQueueStore.replaceKinds(
        ...
        items: [
          ...
          OfflineMutationQueueItem(
            ...
            createdAt: seededAt,
            payload: const {'name': 'x', 'category': 'y'},
            ownerUserId: 'user_b',
          ),
          OfflineMutationQueueItem(
            id: 'legacy_no_owner',
            ...
            createdAt: seededAt,
            payload: const {'business_id': 'b2', 'reason': 'spam'},
          ),
        ],
      );

      final dropped = await OfflineMutationQueueStore.dropForUserMismatch(
        keepUserId: 'user_b',
      );

      expect(dropped, 1);
      final remainingIds = (await OfflineMutationQueueStore.readAll(
        limit: 10,
      )).map((item) => item.id).toSet();
      expect(remainingIds, {'owned_by_b', 'legacy_no_owner'});
      expect(remainingIds.contains('owned_by_a'), isFalse);
    });

    test(
      'classifies price suggestion cooldown errors as resolved conflicts',
      () {
        ...
        expect(sameItem.disposition, OfflineMutationFailureDisposition.resolve);
        expect(
