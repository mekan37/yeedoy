class BusinessMenu {
  ...
  bool get isNative   => !isExternal && !isPhoto && !isQr;
  ...
  factory BusinessMenu.fromMap(Map<String, dynamic> map) {
    return BusinessMenu(
      ...
      externalUrl: _asString(map, ['external_url']),
      sourceImageUrl: _asString(map, ['source_image_url']),
    );
  }
...
class MenuItem {
  ...
  /// Kalori aralığı (kcal) — opsiyonel, AI veya owner tarafından girilir
  final int? caloriesMin;
  final int? caloriesMax;
  /// Porsiyon: "350 g", "2 adet" gibi
  final int? portionSize;
  final String? portionUnit;

  String? get caloriesDisplay {
    if (caloriesMin == null && caloriesMax == null) return null;
    if (caloriesMin != null && caloriesMax != null && caloriesMin != caloriesMax) {
      return '$caloriesMin–$caloriesMax kcal';
    }
    return '${caloriesMin ?? caloriesMax} kcal';
  }

  String? get portionDisplay {
    if (portionSize == null) return null;
    final unit = portionUnit ?? 'g';
    return '$portionSize $unit';
  }
  ...
  factory MenuItem.fromMap(Map<String, dynamic> map) {
    ...
    return MenuItem(
      ...
      allergens: _asStringList(map, ['allergens', 'allergen_codes']),
      caloriesMin: _asInt(map, ['calories_min']),
      caloriesMax: _asInt(map, ['calories_max']),
      portionSize: _asInt(map, ['portion_size']),
      portionUnit: _asString(map, ['portion_unit']),
