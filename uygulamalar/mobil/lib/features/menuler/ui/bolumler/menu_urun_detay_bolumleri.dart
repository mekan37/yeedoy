...
/// Karusel'den açılan viewer: ana imageUrl'i dummy MenuItemPhoto'ya çevirir,
/// topluluk fotoğraflarıyla birleştirir ve istenen index'ten başlatır.
void _openPhotoViewerFromCarousel(
  BuildContext context, {
  String? imageUrl,
  required List<MenuItemPhoto> communityPhotos,
  required int startIndex,
}) {
  final allPhotos = <MenuItemPhoto>[
    if (imageUrl != null && imageUrl.isNotEmpty)
      MenuItemPhoto(id: '__main__', url: imageUrl, urlLarge: imageUrl, urlThumb: imageUrl),
    ...communityPhotos,
  ];
  if (allPhotos.isEmpty) return;
  final safeIndex = startIndex.clamp(0, allPhotos.length - 1);
  Navigator.of(context).push(
    MaterialPageRoute(
      builder: (_) => _PhotoViewerPage(photos: allPhotos, initialIndex: safeIndex),
    ),
  );
}
...
// ─── Fotoğraf Karuseli ────────────────────────────────────────────────────────
//
// Menü ürünü detay sayfasının üstünde gösterilen swipeable fotoğraf galerisi.
// İlk slayt: item.imageUrl (owner tarafından yüklenen ana fotoğraf)
// Sonraki slaylar: topluluk tarafından yüklenen, puana göre sıralı fotoğraflar
// Boş durum: gösterilmez (caller null kontrol yapar)
class MenuItemPhotoCarousel extends StatefulWidget {
  const MenuItemPhotoCarousel({
    super.key,
    this.mainImageUrl,
    this.communityPhotos = const [],
    this.onTapPhoto,
  });

  /// Owner'ın yüklediği ana ürün fotoğrafı (MenuItem.imageUrl)
  final String? mainImageUrl;

  /// Topluluk fotoğrafları — puanlı, sıralı olarak verilmeli
  final List<MenuItemPhoto> communityPhotos;

  /// Fotoğrafa tıklanınca tam ekran görüntüleyiciyi açar
  final void Function(int index)? onTapPhoto;

  bool get hasPhotos =>
      (mainImageUrl != null && mainImageUrl!.isNotEmpty) ||
      communityPhotos.isNotEmpty;

  @override
  State<MenuItemPhotoCarousel> createState() => _MenuItemPhotoCarouselState();
}

class _MenuItemPhotoCarouselState extends State<MenuItemPhotoCarousel> {
  ...
  List<String> get _allUrls {
    ...
    if (widget.mainImageUrl != null && widget.mainImageUrl!.isNotEmpty) {
      urls.add(widget.mainImageUrl!);
    }
    for (final p in widget.communityPhotos) {
      final u = p.urlLarge.isNotEmpty ? p.urlLarge : p.url;
      ...
      child: AspectRatio(
        ...
        child: Stack(
          children: [
            ...
            // Dot indikatörü — birden fazla fotoğraf varsa göster
            if (urls.length > 1)
              Positioned(
                left: 0,
                right: 0,
