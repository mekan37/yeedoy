...
import '../domain/katki_gecmisi.dart';
import '../domain/gunluk_mikro_gorev.dart';
import '../domain/profil_ilerleme.dart';
import '../domain/profil_istatistikleri.dart';
import '../domain/kullanici_koruma_sinyalleri.dart';
import '../domain/haftalik_gorevler.dart';
import 'profil_modeli.dart';
...
class ProfileRepository {
  ...
  Future<Profile?> fetchMyProfile() async {
    ...
    try {
      final row = await _supabase
          .from('user_profiles')
          .select('user_id,display_name,city,district,phone')
          .eq('user_id', uid)
          .single();
  ...
  Future<void> upsertMyProfile(Profile p) async {
    ...
    if (p.city     != null && p.city!.isNotEmpty)     upsertData['city']     = p.city;
    if (p.district != null && p.district!.isNotEmpty) upsertData['district'] = p.district;
    if (p.phone    != null && p.phone!.isNotEmpty)    upsertData['phone']    = p.phone;
    await _supabase.from('user_profiles').upsert(upsertData, onConflict: 'user_id');
  }
  ...
  Future<int> fetchMyReputationScore() async {
    dynamic res;
    try {
      res = await _supabase.rpc('get_my_reputation_score_v1');
    } on PostgrestException catch (e) {
      if (e.code == '42883' ||
          e.message.contains('menu_price_suggestion_status')) {
        return 0;
      }
      rethrow;
    }
  ...
  Future<UserMoatSignals> fetchMyMoatSignals() async {
    Future<dynamic> safeRpc(String name) async {
      try {
        return await _supabase.rpc(name);
      } on PostgrestException catch (e) {
        if (e.code == '42883' ||
            e.message.contains('menu_price_suggestion_status')) {
    ...
    final trustRes = await safeRpc('get_my_trust_graph_v1');
    final segmentRes = await safeRpc('get_my_behavior_segment_v1');
    final silentRes = await safeRpc('get_my_silent_quality_score_v1');
