...
class _RecommendedTabState extends ConsumerState<_RecommendedTab>
  ...
  bool _pendingMapNearbySync = false;
                      ...
                      SliverPadding(
                        ...
                        sliver: SliverList.list(
                          children: [
                            ...
                            // Bugünün Spesiyalleri Bölümü (7→9)
                            BugunSpesiyalBolumu(
                              city: city,
                              lat: st.userLat,
                              lng: st.userLng,
                            ...
                            AppCard(
                              child: Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  // M-23: Aktif filtre sayısı rozeti
                                  Builder(builder: (ctx) {
                                    final aktif = [
                                      st.openNow,
                                      st.minRating > 0,
                                      st.category.isNotEmpty,
                                      st.district.isNotEmpty,
                                      st.recentPriceBoost,
                                      st.mealCardKeys.isNotEmpty,
                                    ].where((v) => v).length;
                                    return Stack(
  ...
  void _syncSurfaceModeFromRoute(BuildContext context) {
    ...
    if (wantedMode == _DiscoverySurfaceMode.map) {
      _syncNearbyModeForMap();
    }
  ...
  void _syncNearbyModeForMap() {
    if (_pendingMapNearbySync) return;
    final st = ref.read(discoverySearchProvider);
    if (st.mode == DiscoveryMode.nearby) return;
    _pendingMapNearbySync = true;
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      _pendingMapNearbySync = false;
      if (!mounted) return;
      final uri = GoRouterState.of(context).uri;
      if (uri.queryParameters['view'] != 'map') return;
      final current = ref.read(discoverySearchProvider);
      if (current.mode == DiscoveryMode.nearby) return;
      await ref
          .read(discoverySearchProvider.notifier)
          .setMode(DiscoveryMode.nearby);
    });
  ...
  void _openBusiness(String businessId, {required String source}) {
    ...
    context.go('/isletme/${Uri.encodeComponent(id)}');
  }
  ...
  Future<void> _openFiltersSheet(
    ...
    var localVeganOnly = st.veganOnly;
    var localGlutenFreeOnly = st.glutenFreeOnly;
                    ...
                    child: SingleChildScrollView(
                      child: Column(
                        ...
                        children: [
                          ...
                          SwitchListTile(
                            contentPadding: EdgeInsets.zero,
                            value: localVeganOnly,
                            title: const Text('Vegan / Vejetaryen seçenekler'),
                            subtitle: const Text(
                              'Vegan veya vejetaryen menü ürünleri olan işletmeler',
                              style: TextStyle(fontSize: 12),
                            ),
                            onChanged: (v) =>
                                setModalState(() => localVeganOnly = v),
                          ),
                          SwitchListTile(
                            contentPadding: EdgeInsets.zero,
                            value: localGlutenFreeOnly,
                            title: const Text('Glutensiz seçenekler'),
                            subtitle: const Text(
                              'Glutensiz menü ürünleri olan işletmeler',
                              style: TextStyle(fontSize: 12),
                            ),
                            onChanged: (v) =>
                                setModalState(() => localGlutenFreeOnly = v),
                          ),
                          const SizedBox(height: 10),
                          Row(
                            children: [
                                        ...
                                        .setFilters(
                                          ...
                                          maxBudgetCents: localMaxBudgetTl <= 0
                                              ...
                                              : (localMaxBudgetTl * 100)
                                                    .round(),
                                          veganOnly: localVeganOnly,
                                          glutenFreeOnly: localGlutenFreeOnly,
                                        );
                                  },
                                  child: Text(t.apply),
                                ),
            ...
            sliver: SliverList.list(
              children: [
                ...
                if (_surfaceMode == _DiscoverySurfaceMode.map) ...[
                  _DiscoveryMapSurface(
                    items: items,
                    isNearby: st.mode == DiscoveryMode.nearby,
                    userLat: st.userLat,
                    userLng: st.userLng,
                    onUseNearby: () => ref
                        .read(discoverySearchProvider.notifier)
                        .setMode(DiscoveryMode.nearby),
                    onOpenBusiness: (id) => _openBusiness(id, source: 'map'),
                  ),
                  const SizedBox(height: 24),
                ],
                Row(
