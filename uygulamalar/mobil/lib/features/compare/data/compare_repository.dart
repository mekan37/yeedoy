...
final compareRepositoryProvider = Provider<CompareRepository>((ref) {
  ...
  return CompareRepository(client);
});
...
class CompareRepository {
  CompareRepository(this.client);
  final SupabaseClient client;

  Future<List<CompareBusiness>> getCompareBusinesses(List<String> ids) async {
    if (ids.isEmpty) return const [];

    final compareRes = await client.rpc(
      'get_business_compare_v1',
      params: {'p_business_ids': ids},
    );
