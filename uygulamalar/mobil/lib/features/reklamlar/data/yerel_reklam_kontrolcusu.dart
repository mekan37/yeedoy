...
class NativeAdController extends Notifier<int> {
  ...
  void load() {
    if (!AdConfig.isNativeEnabled || _disposed || _loadingDisabled) return;
    _warmPool();
  }
  ...
  void _warmPool() {
    if (_disposed || _loadingDisabled) return;
  ...
  void _loadOne() {
    try {
      final ad = NativeAd(
        ...
        listener: NativeAdListener(
          ...
          onAdFailedToLoad: (adObj, error) {
            ...
            if (kDebugMode) {
              debugPrint(
                'native_ad load failed: ${error.code} ${error.message}',
              );
            }
            _warmPool();
          },
      ...
      if (kDebugMode) {
        debugPrint('native_ad fatal setup error: $error');
