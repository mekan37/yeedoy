...
import 'uygulama_hata_kodlari.dart';
...
class AppErrorMapper {
  ...
  static String? _mapError(String raw) {
    ...
    if (raw.contains(AppErrorCodes.reasonRequired)) {
      return 'Bu işlem için neden girmen zorunlu.';
    }
    ...
    if (raw.contains(AppErrorCodes.edgeGuardRequired)) {
      return 'Güvenlik doğrulaması gerekli. Lütfen yeniden dene.';
    }
    ...
    if (raw.contains(AppErrorCodes.offlineQueued)) {
      return 'Bağlantı zayıf. İşlem sıraya alındı, internet gelince otomatik gönderilecek.';
    }
    if (raw.contains(AppErrorCodes.contactVerificationRequired)) {
      return 'Bu işlem için e-posta veya telefon doğrulaması gerekli.';
    }
    if (raw.contains(AppErrorCodes.locationRequired)) {
      return 'Konum gerekli.';
    }
    if (raw.contains(AppErrorCodes.locationPermissionRequired)) {
