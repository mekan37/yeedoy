...
import { normalizeBusinessMenuPathKey } from '@/src/lib/isletme-yolu';
export { normalizeBusinessMenuPathKey } from '@/src/lib/isletme-yolu';
...
export function buildQrHref(input: {
  ...
  return `/karekod/${input.businessId}${toSearchParams({
    lang: input.lang,
    theme: input.theme,
  })}`;
}
...
export function buildShortHref(input: {
  ...
  return `/kod/${input.code}${toSearchParams({
    lang: input.lang,
    theme: input.theme,
  })}`;
}
