...
export function PanelTopbar({ title, toggleButton, actions }: PanelTopbarProps) {
  return (
    <header className="flex h-[60px] shrink-0 items-center gap-3 border-b border-border bg-card px-4">
      {toggleButton}
      {title && (
        <span className="text-sm font-[800] text-textStrong truncate">{title}</span>
      )}
...
export function TopbarIconButton({ onClick, label, children, className }: TopbarIconButtonProps) {
  return (
    <button
      ...
      aria-label={label}
      className={clsx(
        'flex h-9 w-9 items-center justify-center rounded-lg text-muted transition-colors hover:bg-black/[0.06] hover:text-textStrong focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30',
        className,
      )}
    >
      {children}
