...
const NAV_ITEMS: NavItem[] = [
  ...
  { href: '/owner/businesses', label: 'İşletmelerim', icon: <BuildingIcon /> },
  { href: '/owner/menus', label: 'Menü Yönetimi', icon: <MenuIcon /> },
  { href: '/owner/photos', label: 'Fotoğraflar', icon: <PhotoIcon /> },
  { href: '/owner/reviews', label: 'Yorumlar', icon: <StarIcon />, badge: 0 },
  { href: '/owner/reservations', label: 'Rezervasyonlar', icon: <CalendarIcon />, badge: 'Yeni' },
  ...
  { href: '/owner/notifications', label: 'Bildirimler', icon: <BellIcon /> },
...
export function OwnerDashboardShell({ children, business, user, reviewBadgeCount, bannerSlot }: Props) {
  ...
  // Badge yokolur kullanıcı yorumlar sayfasındayken
  const effectiveReviewBadge = pathname?.startsWith('/owner/reviews') ? 0 : (reviewBadgeCount ?? 0);
  const navItems: NavItem[] = NAV_ITEMS.map((item) =>
    item.href === '/owner/reviews' && effectiveReviewBadge > 0
      ? { ...item, badge: effectiveReviewBadge }
      : item
  );
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge != null && (typeof item.badge === 'string' || Number(item.badge) > 0) && (
