...
export default async function BusinessReviewsPage({ params, searchParams }: Props) {
  ...
  const { data: reviews, count } = await (supabase as any).rpc(
    ...
    { count: 'exact' },
  ) as { data: Yorum[] | null; count: number | null };
  ...
  function sortHref(s: SortOption) {
    ...
    return `/isletme/${slug}/yorumlar${qs ? `?${qs}` : ''}`;
  }
...
function YorumKarti({ review: r }: { review: Yorum }) {
    ...
    <article className="rounded-[20px] border border-border bg-cardAlt p-5 shadow-yd1">
      ...
      {(r.r_taste || r.r_service || r.r_price_value || r.r_atmosphere) && (
        <div className="mt-3 flex flex-wrap gap-2">
          {r.r_taste != null && <AltPuan label="Lezzet" value={r.r_taste} />}
          {r.r_service != null && <AltPuan label="Servis" value={r.r_service} />}
          {r.r_price_value != null && <AltPuan label="Fiyat/Değer" value={r.r_price_value} />}
          {r.r_atmosphere != null && <AltPuan label="Atmosfer" value={r.r_atmosphere} />}
        </div>
      )}
...
function YildizSatiri({ rating }: { rating: number }) {
  return (
...
function AltPuan({ label, value }: { label: string; value: number }) {
  return (
