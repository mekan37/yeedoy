/**
 * Route-segment loading UI for /m/[slug].
 *
 * Shown by Next.js while the public menu page Server Component is streaming.
 * Mirrors the visual structure of the menu page to minimise CLS.
 */
export default function PublicMenuSlugLoading() {
  return (
    <main className="mx-auto min-h-screen w-full max-w-7xl px-4 py-5 sm:px-6 sm:py-6">
      {/* Hero card skeleton */}
      <section className="overflow-hidden rounded-[32px] border border-border bg-card shadow-yd2">
        <div className="h-[280px] animate-pulse bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,0.3),_transparent_42%),linear-gradient(135deg,_rgb(var(--yd-color-primary-rgb)),_rgb(var(--yd-color-primary-strong-rgb)))]" />
        <div className="flex items-center gap-4 px-6 py-4">
          <div className="h-14 w-14 animate-pulse rounded-full bg-border" />
          <div className="flex-1 space-y-2">
            <div className="h-5 w-40 animate-pulse rounded-lg bg-border" />
            <div className="h-3 w-24 animate-pulse rounded-md bg-border" />
          </div>
        </div>
      </section>

      {/* Category nav skeleton */}
      <section className="sticky top-3 z-20 mt-6 rounded-[28px] border border-border bg-card/90 p-3 shadow-yd2 backdrop-blur">
        <div className="mb-3 h-4 w-32 animate-pulse rounded-xl bg-border" />
        <div className="flex gap-2 overflow-hidden">
          {[28, 32, 24, 36, 28].map((w, i) => (
            <div
              key={i}
              className="h-10 animate-pulse rounded-full bg-border"
              style={{ width: `${w * 4}px` }}
            />
          ))}
        </div>
      </section>

      {/* Info cards */}
      <section className="mt-6 grid gap-3 md:grid-cols-[1.25fr_0.75fr]">
        <div className="h-28 animate-pulse rounded-[28px] border border-border bg-card shadow-yd1" />
        <div className="h-28 animate-pulse rounded-[28px] border border-border bg-card shadow-yd1" />
      </section>

      {/* Menu items skeleton */}
      <section className="mt-6 grid gap-6 lg:grid-cols-[0.75fr_1.25fr]">
        <div className="space-y-4">
          <div className="h-52 animate-pulse rounded-[28px] border border-border bg-card shadow-yd1" />
          <div className="h-44 animate-pulse rounded-[28px] border border-border bg-card shadow-yd1" />
        </div>
        <div className="space-y-3">
          {[44, 44, 44].map((h, i) => (
            <div
              key={i}
              className="animate-pulse rounded-[28px] border border-border bg-card shadow-yd1"
              style={{ height: `${h * 4}px` }}
            />
          ))}
        </div>
      </section>
    </main>
  );
}
