...
export type CampaignType = 'discount' | 'special_offer' | 'loyalty' | 'announcement';
...
export function CampaignForm({ businessId, campaign, onClose }: Props) {
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Durum" required>
              <select name="status" defaultValue={campaign?.status ?? 'draft'} className={inputCls} required>
                {(Object.entries(STATUS_LABELS) as [CampaignStatus, string][]).map(([v, l]) => (
                  <option key={v} value={v}>{l}</option>
                ))}
              </select>
            </Field>
