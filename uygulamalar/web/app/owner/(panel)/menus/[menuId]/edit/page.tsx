...
export default async function MenuEditorPage({ params }: Props) {
  ...
  const { data: biz } = await (supabase as any).from('businesses').select('id, name').eq('id', menu.business_id).eq('owner_id', user!.id).single() as { data: { id: string; name: string } | null };
  if (!biz) notFound();
  ...
  if (sectionIds.length > 0) {
    const { data: itemsRaw } = await (supabase as any).from('menu_items').select('id, name, description, price_cents, currency, is_available, section_id, sort_order').in('section_id', sectionIds).order('sort_order') as { data: Item[] | null };
    items = itemsRaw ?? [];
  }
