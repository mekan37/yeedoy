...
import { SettingsClient, type BusinessData } from './settings-client';
...
export default async function OwnerSettingsPage() {
  ...
  if (authError || !authData.user) {
    redirect('/owner/login?redirect=/owner/settings');
  }
  ...
  if (!claimData?.business_id) redirect('/owner/dashboard');
  ...
  return (
    <div className="flex flex-col">
      ...
      />
      <PanelContentSurface className="pt-2 sm:pt-4">
        <SettingsClient
          user={{
            id: user.id,
            email: user.email ?? '',
