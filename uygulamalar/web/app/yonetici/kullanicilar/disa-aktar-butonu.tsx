...
import { kullanicilarCsvOlustur, type KullaniciSatiri } from './kullanicilar-yardimcilari';

export function DisaAktarButonu({ rows }: { rows: KullaniciSatiri[] }) {
  function disaAktar() {
    const csv = kullanicilarCsvOlustur(rows);
  ...
  return (
    ...
    >
      <span>Kullanıcı Dışa Aktar<span className="block text-[10px] font-bold text-muted">Kullanıcı listesini dışa aktarın (CSV)</span></span>
      <DownloadIcon />
    </button>
  );
}
