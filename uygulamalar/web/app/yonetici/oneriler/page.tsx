...
export default async function AdminSuggestionsPage({ searchParams }: Props) {
  const yetkili = await hasPermission('page:oneriler');
  if (!yetkili) {
    return (
      <div className="flex flex-col">
        <PanelSayfaBasligi eyebrow="Yönetici" title="Öneriler" description="Bu sayfayı görüntüleme yetkiniz yok." />
        <PanelIcerikYuzeyi className="pt-6"><YetkisizErisim sayfaAdi="Öneriler" /></PanelIcerikYuzeyi>
      </div>
    );
  }
