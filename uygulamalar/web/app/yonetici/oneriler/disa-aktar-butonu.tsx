...
import { onerilerCsvOlustur, type OneriSatiri } from './oneriler-yardimcilari';

export function DisaAktarButonu({ rows }: { rows: OneriSatiri[] }) {
  function disaAktar() {
    const csv = onerilerCsvOlustur(rows);
    const blob = new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `oneriler-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
