  ...
  return (
    <div className="flex flex-col">
      ...
      <PanelIcerikYuzeyi className="pt-6">
        {/* Ozet KPI kartlari — gercek RPC verisi */}
        <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {[
            ...
            { label: 'Acik Aday', value: summary.open_leads, cls: 'text-primary bg-primary/8' },
            { label: 'Gosterim (30g)', value: summary.impressions_30d.toLocaleString('tr-TR'), cls: 'text-blue-700 bg-blue-50' },
            { label: 'Tekil Kullanici', value: summary.unique_users_30d.toLocaleString('tr-TR'), cls: 'text-purple-700 bg-purple-50' },
            {
              label: 'Est. Gelir',
              value:
                summary.estimated_active_revenue_cents > 0
                  ? new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY', minimumFractionDigits: 0 }).format(
                      summary.estimated_active_revenue_cents / 100,
                    )
                  : '—',
            ...
            <div key={label} className="rounded-xl border border-border bg-card p-4">
              ...
              <p className={`mt-1 text-lg font-[900] ${cls} rounded-lg px-2 py-0.5 inline-block`}>{value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mb-5 flex flex-wrap gap-2">
          <Link
            href="/yonetici/sponsor-adaylari"
            className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-[700] text-textStrong hover:border-primary/30 hover:bg-cardAlt/50 transition-colors"
          >
            Adaylar ({summary.open_leads} acik) →
          </Link>
          <Link
            href="/yonetici/sponsor-paketleri"
            className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-[700] text-textStrong hover:border-primary/30 hover:bg-cardAlt/50 transition-colors"
          >
            Paketleri Yon. →
          </Link>
        ...
        <div className="mb-4 flex flex-wrap gap-3">
          ...
          <form method="get" className="flex flex-wrap gap-2">
            ...
            {[
              { v: 'all', l: 'Tum Yuzeyl.' },
              { v: 'discovery', l: 'Kesif' },
              { v: 'business_page', l: 'Isletme' },
              { v: 'stories', l: 'Hikayeler' },
              { v: 'verified', l: 'Dogrulanmis' },
              { v: 'premium', l: 'Premium' },
            ].map(({ v, l }) => (
              <button
                key={v}
        ...
        {list.length === 0 ? (
          <PanelEmptyState icon={<StarIcon />} title="Sponsorluk bulunamadi" description="Secili filtre icin kayit yok." />
        ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left">
                    {['Isletme', 'Paket', 'Yuzey', 'Baslangic', 'Bitis', 'Gunluk Kap', 'Durum'].map((h) => (
                      <th
                        key={h}
                        className="px-4 py-3 text-[11px] font-[800] uppercase tracking-wide text-muted whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-border">
                  {list.map((s) => (
                    <tr key={s.sponsorship_id} className="hover:bg-black/[0.01]">
                      ...
                      <td className="px-4 py-3 text-sm text-muted">{s.package_id ? s.package_id.slice(0, 8) + '…' : '—'}</td>
                      <td className="px-4 py-3">
