...
import { zincirlerCsvOlustur, type ZincirSatiri } from './zincirler-yardimcilari';

export function DisaAktarButonu({ rows }: { rows: ZincirSatiri[] }) {
  function disaAktar() {
    const csv = zincirlerCsvOlustur(rows);
    const blob = new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `zincirler-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
