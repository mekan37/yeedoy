...
export function PushKampanyaFormu({
  ...
  if (sent) return (
    <div className="flex flex-col items-center gap-3 py-8">
      ...
      <p className="font-[800] text-textStrong">Kampanya gönderildi!</p>
      <button onClick={() => setSent(false)} className="text-sm font-[600] text-primary">Yeni Kampanya Oluştur</button>
    </div>
  );
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <select
          ...
          className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-textStrong focus:border-primary focus:outline-none"
        >
          {businesses.length === 0 ? (
      ...
      <div>
        <p className="mb-2 text-xs font-[700] text-muted">Hedef Segment</p>
        <div className="grid gap-2 sm:grid-cols-2">
          {SEGMENTS.map(s => (
            ...
            >
              ...
              <div>
                <p className="text-sm font-[700] text-textStrong">{s.label}</p>
                <p className="text-xs text-muted">{s.desc}</p>
                <p className="mt-1 text-xs font-[800] text-primary">~{(estimatedReach[s.value] ?? 0).toLocaleString('tr-TR')} kişi</p>
              </div>
            </label>
          ))}
        </div>
      ...
      {/* Title */}
      <div>
        <label className="mb-1 block text-xs font-[700] text-muted">Bildirim Başlığı</label>
        <input
          value={title}
          onChange={e => setTitle(e.target.value)}
          maxLength={80}
          placeholder="Ör: Yeni menüler keşfedin!"
          className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-textStrong placeholder:text-muted focus:border-primary focus:outline-none"
        />
      ...
      {/* Body */}
      <div>
        <label className="mb-1 block text-xs font-[700] text-muted">Bildirim İçeriği</label>
        <textarea
          value={body}
