...
export async function markLeadContactedAdmin(formData: FormData): Promise<void> {
  ...
  revalidatePath(`/admin/sponsorship-leads?status=${currentStatus}`);
  revalidatePath('/admin/sponsorship-leads');
  revalidatePath('/admin/sponsorships');
}

export async function closeLeadAdmin(formData: FormData): Promise<void> {
  const id = String(formData.get('id') ?? '');
  const currentStatus = String(formData.get('currentStatus') ?? 'new');
  if (!id) return;
