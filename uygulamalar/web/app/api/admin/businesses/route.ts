...
export async function GET(request: Request) {
  ...
  const rl = rateLimit(`admin-business-list:${identity}`, 60, 60_000);
  if (!rl.ok) {
    return NextResponse.json({ error: 'rate_limited' }, { status: 429 });
  }
  ...
  const pageSize = Math.min(100, Math.max(1, parseInt(url.searchParams.get('page_size') ?? '40', 10)));
  ...
  if (error) {
    logger.warn('Admin business list failed', { error: error.message });
