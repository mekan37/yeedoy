    ...
    <main className="min-h-screen bg-bg">
      <div className="mx-auto max-w-2xl px-4 py-12">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/group-requests/new" className="rounded-xl bg-primary px-4 py-2 text-sm font-[700] text-white cursor-pointer">+ Yeni İstek</Link>
        </div>
        {list.length === 0 ? (
          <div className="rounded-2xl border border-border bg-card p-10 text-center">
            <p className="font-[700] text-textStrong mb-2">Henüz grup isteği oluşturmadınız</p>
            <Link href="/group-requests/new" className="text-sm text-primary hover:underline cursor-pointer">İlk grup isteğinizi oluşturun →</Link>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {list.map(r => {
              ...
              return (
                <Link key={r.id} href={`/group-requests/${r.id}`} className="flex items-center justify-between rounded-2xl border border-border bg-card px-5 py-4 transition-colors hover:border-primary/30 cursor-pointer">
                  <div>
