...
import { ReservationsClient, type DbReservation } from './rezervasyonlar-istemcisi';
...
export default async function OwnerReservationsPage() {
  ...
  const { data: result, error: rpcError } = await (supabase).rpc('owner_list_reservations_v1', {
    p_business_id: claim.business_id,
    p_limit: 200,
    p_offset: 0,
  }) as unknown as { data: { rows: DbReservation[]; total: number } | null; error: { message: string } | null };
