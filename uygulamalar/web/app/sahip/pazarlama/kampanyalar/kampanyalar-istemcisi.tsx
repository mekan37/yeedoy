...
import type { Kampanya, KampanyaTipi, KampanyaDurumu } from './kampanya-formu';
import { kampanyaSil } from './kampanya-islemleri';
...
const TIP_RENKLERI: Record<KampanyaTipi, string> = {
...
const DURUM_RENKLERI: Record<KampanyaDurumu, string> = {
  draft:     'bg-zinc-100 text-zinc-600',
  planned:   'bg-blue-50 text-blue-700',
  active:    'bg-green-50 text-green-700',
  completed: 'bg-zinc-100 text-zinc-500',
};
const SEKMELER: { key: KampanyaDurumu | 'all'; label: string }[] = [
  { key: 'all',       label: 'Tümü'       },
...
export function KampanyalarIstemcisi({ businessId, initialCampaigns, initialTotal, stats }: Props) {
  ...
  const tabCounts = SEKMELER.reduce<Record<string, number>>((acc, tab) => {
  ...
  function handleDelete(id: string) {
    ...
    startTransition(async () => {
      await kampanyaSil(id, businessId);
