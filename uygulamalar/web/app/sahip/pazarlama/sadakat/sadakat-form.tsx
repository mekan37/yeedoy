...
export function SadakatProgramFormu({ businesses }: { businesses: Array<{ id: string; name: string }> }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-bold text-textStrong">Program Adı</label>
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-bold text-textStrong">Damga Sayısı: {stamps}</label>
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-bold text-textStrong">Ödül</label>
      ...
      {result && (
        <p className={`rounded-xl border px-4 py-2.5 text-sm font-bold ${result.ok ? 'border-success/20 bg-success/8 text-success' : 'border-danger/20 bg-danger/8 text-danger'}`}>
          {result.msg}
        </p>
      )}
      <button type="submit" disabled={loading || !name.trim() || !reward.trim()} className="btn-primary rounded-xl px-4 py-3 text-sm font-extrabold text-white disabled:opacity-50">
