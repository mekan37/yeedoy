...
export default async function MusterilerSayfasi() {
  ...
  const [{ data: customersRaw }, { data: businessChain }, { data: program }] = await Promise.all([
    sb.rpc('get_business_customers_v1', { p_business_id: businessId }) as unknown as Promise<{ data: MusteriOzet[] | null }>,
    sb.from('businesses').select('chain_id, chains(name)').eq('id', businessId).maybeSingle(),
    sb.from('loyalty_programs').select('id').eq('business_id', businessId).maybeSingle(),
  ]);
  ...
  type OlayZamani = { created_at: string | null };
  const [{ data: pageViews }, { data: reservationRows }, { data: loyaltyEventRows }] = await Promise.all([
    sb.from('analytics_events').select('created_at').eq('business_id', businessId).eq('event_name', 'business_page_view').gte('created_at', since),
    sb.from('reservations').select('created_at').eq('business_id', businessId).gte('created_at', since),
    (program?.id
      ...
      : Promise.resolve({ data: [] as OlayZamani[] })),
  ]);

  function gunlukSeri(rows: Array<{ created_at: string | null }> | null): Record<string, number> {
