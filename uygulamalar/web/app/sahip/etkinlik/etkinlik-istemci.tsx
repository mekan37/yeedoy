    ...
    <div className="flex flex-col gap-4">
      ...
      {showCreate && (
        <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5">
          <p className="mb-4 font-[800] text-textStrong">Yeni Etkinlik Oluştur</p>
          <div className="grid gap-3 sm:grid-cols-2">
            {bizIds.length > 1 && (
              <div className="sm:col-span-2">
                <label className="mb-1 block text-xs font-[700] text-muted">İşletme</label>
            ...
            <div className="sm:col-span-2">
              <label className="mb-1 block text-xs font-[700] text-muted">Etkinlik Adı *</label>
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Ör: Aralık Müzik Gecesi"
                className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm focus:border-primary focus:outline-none" />
            </div>
            <div className="sm:col-span-2">
              <label className="mb-1 block text-xs font-[700] text-muted">Açıklama</label>
              <textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}
                className="w-full resize-none rounded-lg border border-border bg-surface px-3 py-2 text-sm focus:border-primary focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-[700] text-muted">Tarih ve Saat *</label>
              <input type="datetime-local" value={eventDate} onChange={e => setEventDate(e.target.value)}
                className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm focus:border-primary focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-[700] text-muted">Kapasite (opsiyonel)</label>
              <input type="number" min={0} value={capacity} onChange={e => setCapacity(e.target.value)} placeholder="Sınırsız"
                className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm focus:border-primary focus:outline-none" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-[700] text-muted">Bilet Fiyatı ₺ (opsiyonel)</label>
              <input type="number" min={0} step={0.01} value={ticketPrice} onChange={e => setTicketPrice(e.target.value)} placeholder="Ücretsiz"
                className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm focus:border-primary focus:outline-none" />
            </div>
          </div>
          {error && <p className="mt-2 text-xs font-[700] text-red-600">{error}</p>}
          <div className="mt-4 flex gap-2">
            <button onClick={submit} disabled={isPending}
              className="rounded-xl bg-primary px-4 py-2 text-sm font-[800] text-white hover:bg-primary/90 disabled:opacity-50">
              {isPending ? 'Kaydediliyor…' : 'Oluştur'}
            </button>
            <button onClick={() => setShowCreate(false)} className="rounded-xl border border-border px-4 py-2 text-sm font-[700] text-muted hover:text-textStrong">İptal</button>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-zinc-50 dark:bg-zinc-900/40 text-left">
                ...
                <th className="px-5 py-3 text-[11px] font-[800] uppercase tracking-wide text-muted">Bilet</th>
            ...
            <tbody className="divide-y divide-border">
              {displayed.map(ev => {
                ...
                return (
                  <tr key={ev.id} className="hover:bg-black/[0.02]">
                    <td className="px-5 py-3">
                      <p className="font-[700] text-textStrong">{ev.title}</p>
                      ...
                      {ev.capacity ? (
                        <div>
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs font-[700] text-textStrong">{ev.tickets_sold}/{ev.capacity}</span>
                          </div>
                          {isFull && <span className="text-[10px] font-[700] text-red-600">DOLU</span>}
                        </div>
                      ) : (
                    ...
                    <td className="px-5 py-3">
                      <span className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-[700] ${ev.status === 'active' ? 'bg-green-50 text-green-700' : ev.status === 'cancelled' ? 'bg-red-50 text-red-700' : 'bg-zinc-100 text-zinc-500'}`}>
                    ...
                    <td className="px-5 py-3">
                      {ev.status === 'active' && (
                        <button
                          ...
                          className="rounded-lg border border-red-200 px-2 py-1 text-[10px] font-[700] text-red-600 hover:bg-red-50 disabled:opacity-50"
                        >
                          İptal
                        </button>
                      )}
