...
export function BrandingEditor({ businessId, initialLogoUrl, initialCoverUrl, businessName }: Props) {
  ...
  async function handleUpload(file: File, type: 'logo' | 'cover') {
    ...
    try {
      ...
      if (!response.ok) {
        setError(
          data?.error === 'rate_limited' ? 'Çok fazla istek, bekleyin.' :
          data?.error === 'file_too_large' ? 'Dosya çok büyük.' : 'Yükleme başarısız.',
        );
        return;
      }

      if (!data?.url) throw new Error('invalid_upload_response');

      if (type === 'logo') { setLogoUrl(data.url); setLogoV((v) => v + 1); }
      else                 { setCoverUrl(data.url); setCoverV((v) => v + 1); }
    } catch (uploadError) {
      setError(
