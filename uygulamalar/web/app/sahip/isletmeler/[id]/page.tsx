...
import { BrandingEditor } from './marka-editoru';
import { MealCardEditor } from './yemek-karti-editoru';
...
export default async function BusinessDetailPage({ params }: Props) {
  ...
  // Meal card sorguları auth bağlamı gerektiriyor → mevcut auth client (supabase)
  ...
  const allProviders: MealCardProvider[] = (allProvidersResult.data ?? []).map((row: MealCardProviderRow) => ({
    id: row.id, key: row.key, name: row.name, assetName: row.asset_name, sortOrder: row.sort_order,
  }));
  const selectedKeys: string[] = (selectedProvidersResult.data ?? []).map((row: SelectedMealCardRow) => row.key);
