      ...
      <form onSubmit={handleSearch} className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="h-14 w-full rounded-2xl border border-[#e5e7eb] bg-[#fafafa] pl-12 pr-4 text-base text-[#111827] placeholder:text-[#d1d5db] outline-none transition focus:border-[#dc2626] focus:bg-white focus:ring-2 focus:ring-[#dc2626]/10"
          />
        </div>
        <button
          type="submit"
          disabled={isPending || !query.trim()}
          className="h-14 rounded-2xl bg-[#dc2626] px-7 text-sm font-[800] text-white shadow-[0_2px_8px_rgba(220,38,38,0.25)] transition hover:bg-[#b91c1c] disabled:opacity-60"
        >
          {isPending ? (
          ...
          {results.length === 0 ? (
            ...
            <div className="overflow-hidden rounded-2xl border border-[#f0f0f0] bg-white shadow-sm">
              {results.map((biz, i) => (
                ...
                >
                  <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-[#fef2f2] text-base font-[900] text-[#dc2626]">
                    {biz.name[0].toUpperCase()}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-[800] text-[#111827] truncate">{biz.name}</p>
