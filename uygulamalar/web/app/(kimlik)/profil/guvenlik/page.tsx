  ...
  return (
    <main className="min-h-screen bg-bg">
      <div className="mx-auto max-w-lg px-4 py-12">
        ...
        <h1 className="mb-8 text-2xl font-black text-textStrong">Hesap Güvenliği</h1>

        <div className="flex flex-col gap-4">
          {/* Account info */}
          <div className="rounded-2xl border border-border bg-card p-6">
            <h2 className="mb-4 text-base font-black text-textStrong">Hesap Bilgileri</h2>
            <div className="flex flex-col gap-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted">E-posta</span>
                <span className="font-bold text-textStrong">{email}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted">Giriş yöntemi</span>
                <span className="font-bold text-textStrong capitalize">{provider === 'email' ? 'E-posta' : provider}</span>
              </div>
              {lastSignIn && (
                <div className="flex justify-between">
                  <span className="text-muted">Son giriş</span>
                  <span className="font-bold text-textStrong">{lastSignIn}</span>
                </div>
              )}
              {createdAt && (
                <div className="flex justify-between">
                  <span className="text-muted">Üyelik tarihi</span>
                  <span className="font-bold text-textStrong">{createdAt}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Password */}
          <div className="rounded-2xl border border-border bg-card p-6">
            <h2 className="mb-1 font-black text-textStrong">Şifre Değiştir</h2>
            <p className="mb-4 text-sm text-muted">E-posta adresinize şifre sıfırlama bağlantısı gönderilir.</p>
            <Link
              href="/sifremi-unuttum"
              className="inline-flex min-h-[40px] items-center rounded-xl border border-border bg-bg px-4 text-sm font-bold text-textStrong hover:border-primary/30"
            >
          ...
          {/* Active session */}
          <div className="rounded-2xl border border-border bg-card p-6">
            <h2 className="mb-1 font-black text-textStrong">Aktif Oturum</h2>
            ...
            <div className="mb-4 flex items-center gap-3 rounded-xl border border-border bg-bg px-4 py-3">
              ...
              <div className="min-w-0">
                <p className="text-sm font-bold text-textStrong">Bu cihaz</p>
                {lastSignIn && <p className="text-xs text-muted">Giriş: {lastSignIn}</p>}
              </div>
              {accessToken && (
                <span className="ml-auto rounded-full bg-success/10 px-2 py-0.5 text-[10px] font-black text-success">Aktif</span>
              )}
            </div>
            {accessToken && (
              <OturumKapatButonu />
          ...
          {/* C-6: 2FA TOTP */}
          <div className="rounded-2xl border border-border bg-card p-6">
            <h2 className="mb-1 font-black text-textStrong">İki Faktörlü Doğrulama (2FA)</h2>
          ...
          {/* Danger zone */}
          <div className="rounded-2xl border border-danger/20 bg-danger/4 p-6">
            <h2 className="mb-1 font-black text-danger">Tehlikeli Bölge</h2>
            <p className="mb-4 text-sm text-muted">Bu işlemler geri alınamaz.</p>
            <Link
              href="/profil/ayarlar"
              className="inline-flex min-h-[40px] items-center rounded-xl border border-danger/30 bg-card px-4 text-sm font-bold text-danger hover:bg-danger/8"
            >
              Hesabı Sil
            </Link>
          </div>
