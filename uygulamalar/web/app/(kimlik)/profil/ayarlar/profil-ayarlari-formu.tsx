...
// ── Ana bileşen ──────────────────────────────────────────────────────────────
//
// Önceden bu bileşenin (o zaman doğrudan page.tsx idi) profil verisi bir
// useEffect içinde mount'tan SONRA çekiliyordu — sayfa her zaman boş bir
// formla açılıp, veri gelince dolduruluyordu (flash of empty content).
// Artık başlangıç değerleri sunucu tarafında (page.tsx, Server Component)
// çekilip prop olarak geçiriliyor; form ilk render'da zaten dolu geliyor.
// Form mantığının kendisi (state, handler'lar) değişmedi — yalnızca ilk
// veri kaynağı client fetch'ten sunucu prop'una taşındı.

export type ProfilAyarlariBaslangic = {
  userId: string;
  email: string;
  firstName: string;
  lastName: string;
...
export function ProfilAyarlariFormu({ initial }: { initial: ProfilAyarlariBaslangic }) {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);

  // Profil state — sunucudan gelen ilk değerlerle başlatılıyor
  ...
  const [hakkinda, setHakkinda]       = useState(initial.hakkinda);
