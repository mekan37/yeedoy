    ...
    <main className="min-h-screen bg-bg">
      <div className="mx-auto max-w-2xl px-4 py-12">
        ...
        <div className="mb-6 flex items-center gap-3">
          <h1 className="text-2xl font-black text-textStrong">Fiyat Alarmlarım</h1>
          {active.length > 0 && (
            <span className="rounded-full bg-primary px-2 py-0.5 text-[11px] font-bold text-white">{active.length} aktif</span>
          )}
        </div>
        ...
        {list.length === 0 ? (
          <div className="rounded-2xl border border-border bg-card p-10 text-center">
            ...
            <p className="font-bold text-textStrong mb-2">Henüz fiyat alarmınız yok</p>
          ...
          <div className="flex flex-col gap-6">
            {active.length > 0 && (
              <section>
                <p className="mb-3 text-xs font-black uppercase tracking-wide text-muted">Aktif Alarmlar</p>
            ...
            {inactive.length > 0 && (
              <section>
                <p className="mb-3 text-xs font-black uppercase tracking-wide text-muted">Pasif</p>
        ...
        <p className="mt-8 text-[11px] text-muted leading-relaxed">
          ...
          <Link href="/bildirim-ayarlari" className="font-bold text-primary hover:underline">bildirim ayarlarından</Link>
    ...
    <div className={`rounded-2xl border bg-card p-5 ${dropped ? 'border-success/40' : 'border-border'}`}>
      <div className="flex items-start gap-4">
        <div className="min-w-0 flex-1">
          <p className="font-black text-textStrong">{item?.name ?? 'Bilinmeyen ürün'}</p>
          {biz && (
            <Link href={`/isletme/${biz.slug}`} className="mt-0.5 block text-[12px] font-bold text-primary hover:underline">
              {biz.name}
            </Link>
          )}
          <div className="mt-2 flex flex-wrap items-center gap-3">
            <div>
              ...
              <p className="text-sm font-black text-textStrong">{fmt(a.target_price_cents, a.currency)}</p>
            </div>
            {a.current_price_cents != null && (
              <div>
                <p className="text-[11px] text-muted">Güncel</p>
                <p className={`text-sm font-black ${dropped ? 'text-success' : 'text-textStrong'}`}>
          ...
          {dropped && (
            <p className="mt-2 inline-flex items-center gap-1 rounded-full bg-success/10 px-2.5 py-1 text-[11px] font-black text-success">
              Fiyat hedefe ulaştı!
            </p>
          )}
        </div>
