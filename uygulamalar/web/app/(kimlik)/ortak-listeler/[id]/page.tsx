...
import { voteOnItem } from './oylama-islemleri';
...
export default async function CollabListDetailPage({ params }: Props) {
  ...
  const shareUrl = list.invite_token
    ? `${process.env.NEXT_PUBLIC_SITE_URL ?? ''}/ortak-listeler/join?token=${list.invite_token}`
    : null;

  return (
    <main className="min-h-screen bg-bg">
      <div className="mx-auto max-w-2xl px-4 py-12">
        <Link href="/ortak-listeler" className="mb-6 inline-flex items-center gap-1.5 text-sm text-muted hover:text-primary cursor-pointer">
          ← Listelerim
        </Link>
