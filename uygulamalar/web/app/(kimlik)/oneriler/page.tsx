  ...
  return (
    <main className="min-h-screen bg-bg">
      <div className="mx-auto max-w-2xl px-4 py-12">
        ...
        <h1 className="mb-2 text-2xl font-black text-textStrong">Önerilerim</h1>
        ...
        {list.length > 0 && (
          <>
            <p className="mb-3 text-xs font-black uppercase tracking-wide text-muted">Geçmiş Öneriler</p>
              ...
              {list.map((s) => {
                ...
                return (
                  <div key={s.id} className="flex items-center justify-between rounded-2xl border border-border bg-card px-5 py-4">
                    ...
                    <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold ${st.className}`}>{st.label}</span>
                  </div>
                );
              })}
            </div>
