    ...
    <main className="min-h-screen bg-bg">
      <div className="mx-auto max-w-2xl px-4 py-12">
        ...
        <div className="mb-6 flex items-center gap-3">
          <h1 className="text-2xl font-[900] text-textStrong">Avantajlarım</h1>
          {active.length > 0 && (
            <span className="rounded-full bg-primary px-2 py-0.5 text-[11px] font-[700] text-white">{active.length}</span>
          )}
        </div>
        ...
        {list.length === 0 ? (
          ...
          <div className="flex flex-col gap-6">
            ...
            {active.length > 0 && (
              <section>
                <p className="mb-3 text-xs font-[900] uppercase tracking-wide text-muted">Kullanılabilir</p>
                      ...
                      <div key={perk.id} className={`rounded-2xl border bg-card p-5 ${expired ? 'border-border opacity-60' : 'border-primary/20'}`}>
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <p className="font-[900] text-textStrong">{perk.title}</p>
                            {perk.description && <p className="mt-0.5 text-sm text-muted">{perk.description}</p>}
                            {perk.businesses && (
                              <Link href={`/isletme/${perk.businesses.slug}`} className="mt-1 block text-[12px] font-[700] text-primary hover:underline">
            ...
            {used.length > 0 && (
              <section>
                <p className="mb-3 text-xs font-[900] uppercase tracking-wide text-muted">Kullanılanlar</p>
                <div className="flex flex-col gap-3">
                  {used.map((perk) => (
                    <div key={perk.id} className="flex items-center justify-between gap-3 rounded-2xl border border-border bg-card px-5 py-4 opacity-60">
                      <div>
                        <p className="text-sm font-[700] text-textStrong">{perk.title}</p>
                        {perk.businesses && <p className="text-[12px] text-muted">{perk.businesses.name}</p>}
                      </div>
                      <span className="rounded-full bg-border px-2.5 py-1 text-[10px] font-[700] text-muted">Kullanıldı</span>
                    </div>
                  ))}
                </div>
              </section>
