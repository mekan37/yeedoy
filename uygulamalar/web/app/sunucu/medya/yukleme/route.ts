...
export async function POST(request: Request) {
  ...
  const limit = await rateLimit(`media-upload:${identity}`, 10, 60_000);
  if (!limit.ok) {
  ...
  const extension = dosyaUzantisi(mimeType);
  const path =
  ...
  const result = await gorselYukle({
    ...
    file,
    path,
    logContext: { businessId: parsed.data.businessId, type: parsed.data.type },
  });
