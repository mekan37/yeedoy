  ...
  return (
    <main className="min-h-screen bg-bg">
      <div className="mx-auto max-w-2xl px-4 py-12">
        <Link href="/feed" className="mb-6 inline-flex items-center gap-1.5 text-sm text-muted hover:text-primary cursor-pointer">
          ← Gurme Akışı
        </Link>
                ...
                return (
                  <article key={r.id} className="rounded-2xl border border-border bg-card p-5">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      {biz ? (
                        <Link href={`/b/${biz.slug}`} className="font-[800] text-textStrong hover:text-primary transition-colors cursor-pointer">
                          {biz.name}
                        </Link>
                      ) : (
