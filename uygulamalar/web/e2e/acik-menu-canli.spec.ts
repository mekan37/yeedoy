...
test.describe.serial('canli acik menu smoke', () => {
  test('acik menu, karekod sayfasi ve yonlendirme akisi calisir', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: /karekod/i })).toHaveAttribute(
      'href',
    ...
    await expect(page.getByRole('heading', { name: /karekod uretimini acmak icin giris yapin/i })).toBeVisible();
  ...
  test('owner can save template settings, upload branding and public menu applies them', async ({
    ...
    try {
      const devir = await devirQrOturumu({
      ...
      expect(devir.ok).toBeTruthy();
      expect(devir.redirectTo).toBe(`/karekod/${target.businessId}?lang=tr&theme=minimal`);
      ...
      await expect(page.getByRole('textbox', { name: /arama the menu/i })).toBeVisible();
      await expect(
