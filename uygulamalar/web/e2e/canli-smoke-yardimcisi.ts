...
export async function devirQrOturumu(input: {
  request: {
  ...
  const response = await input.request.post('/kimlik/panel-devir', {
    headers: {
  ...
  if (!response.ok()) {
    throw new Error(`Panel devir hatasi: ${response.status()} ${await response.text()}`);
  }
