...
`docs/research/` ve `docs/engineering/` klasörlerindeki eski raporlar (MVP Scope Prune Audit, Selective Restore Plan, Loyalty Defer Decision vb.) **tarihsel bağlam ve analiz** sağlarken, **tek başına karar kaynağı değildir**. Bu raporlar 2026 Haziran başında hazırlanmış ve o zamanın varsayımlarını yansıtmaktadır.
