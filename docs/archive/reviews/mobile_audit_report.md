...
- Mobil uygulama gercekte bir consumer app: kesif, isletme, menu, urun, yorum, favori, profil, bildirim ve topluluk katkisi akislari aktif. Kanit: `apps/mobile_flutter/lib/app/router.dart`
- Owner ve admin operasyonlari mobilde yok; `/owner*` ve `/admin*` rotalari mobilde `'/panel-web'` uzerinden panel akisina yonleniyor. Kanit: `apps/mobile_flutter/lib/app/router.dart`
- En guclu bugunku urun omurgasi: discovery -> business -> menu -> item -> verify/report/review -> favorites/profile. Kanit: `apps/mobile_flutter/lib/features/discovery/ui/discovery_page.dart`, `apps/mobile_flutter/lib/features/business/ui/business_page.dart`, `apps/mobile_flutter/lib/features/menus/ui/menu_item_page.dart`
- QR ve public menu baglantisi mobilde tuketim odakli: QR scan / deep-link cozumleme var, fakat QR Studio / owner panel akislari mobilde degil. Kanit: `apps/mobile_flutter/lib/core/linking/yeedoy_route_resolver.dart`, `apps/mobile_flutter/lib/features/contribute/ui/contribute_entry.dart`, `apps/mobile_flutter/lib/features/menus/ui/public_menu_share_page.dart`
- V2/deneysel alanlar zaten kodda mevcut: `smart_feed`, `taste_twin`, `group_requests`, `top_businesses`, `heroes`, `gourmets`, `budget_combos`, `suspended_meals`, `perks`, `embeds`, sponsorlu kesif. Kanit: `apps/mobile_flutter/lib/features/*`
- Mobil bootstrap production-grade temeller tasiyor: Supabase auth, secure local storage, Firebase Analytics/Crashlytics/Performance, Mobile Ads ve dotenv init birlikte calisiyor. Kanit: `apps/mobile_flutter/lib/main.dart`
- Supabase kullanimi yogun ve RPC-centric. Discovery, menu, profile, taste, group requests ve alerts alanlari agirlikla RPC ile okunuyor; bu, backend kontratini dogru dokumante etmeden gelisimi riskli yapar. Kanit: `apps/mobile_flutter/lib/features/**/*repository*.dart`
- Offline destegi artik merkezi hale geliyor; SQLite-backed snapshot store, unified mutation queue, replay servisi, connectivity-restore tetigi ve client-side idempotency standardi var. Cekirdek katki write'larinda explicit server-side idempotency, action-bazli retry/backoff ve devtools queue diagnostics + conflict policy + suggested action operator gorunurlugu saglandi; `favorite`, `price vote`, `follow`, `photo vote`, `price alert create`, `group offer vote` ve `presence submit` de explicit desired-state/idempotent write kapsamina alindi. Panel `/admin/observability` artik `offline_mutation_outcome` event'lerini operator yuzeyinde aciyor; threshold ve escalation persistence kurallari runtime calibration ile deploy'suz ayarlanabiliyor. `offline_write_guard_check.dart` yeni write yuzeylerinin registry disinda kalmasini CI seviyesinde blokluyor. Kanit: `apps/mobile_flutter/lib/core/storage/local_db/sqflite_local_db_store.dart`, `apps/mobile_flutter/lib/core/storage/offline_mutation_queue.dart`, `apps/mobile_flutter/lib/core/storage/offline_mutation_idempotency.dart`, `apps/mobile_flutter/lib/core/storage/offline_sync_service.dart`, `apps/mobile_flutter/lib/core/network/connectivity_restore_service.dart`, `apps/mobile_flutter/lib/core/storage/offline_queue_diagnostics.dart`, `apps/panel_flutter_web/lib/features/admin/ui/admin_observability_page.dart`, `apps/mobile_flutter/tool/offline_write_guard_check.dart`, `supabase/migrations/20260325000004_review_report_idempotency_v1.sql`, `supabase/migrations/20260325000005_business_suggestion_idempotency_v1.sql`, `supabase/migrations/20260325000007_menu_suggestion_idempotency_v1.sql`, `supabase/migrations/20260325000008_secondary_interaction_idempotency_v1.sql`, `supabase/migrations/20260325000009_follow_and_photo_vote_idempotency_v1.sql`, `supabase/migrations/20260325000010_admin_offline_mutation_observability_v1.sql`, `supabase/migrations/20260325000011_price_alert_idempotency_v1.sql`, `supabase/migrations/20260325000012_group_offer_vote_and_presence_idempotency_v1.sql`
- Skor dili 13.6 kapsaminda toparlandi; mobil artik topluluga acik puanlari `kullanici guveni` ve `veri guveni` omurgasina oturtuyor. Profilde destek sinyalleri ayri anlatiliyor, business/menu/item ekranlarinda `trust`, `confidence` ve `value score` kavramlari ortak rehber sheet ile aciklaniyor. Kanit: `apps/mobile_flutter/lib/features/shared/ui/components/community_score_explainer_sheet.dart`, `apps/mobile_flutter/lib/features/profile/ui/profile_page.dart`, `apps/mobile_flutter/lib/features/business/ui/business_page.dart`, `apps/mobile_flutter/lib/features/menus/ui/menu_page.dart`, `apps/mobile_flutter/lib/features/menus/ui/menu_item_page.dart`
...
- Embed / media / OCR tarafi mobil build maliyetini artiriyor: `webview_flutter`, `youtube_player_iframe`, `google_ml_kit`, `image_picker`, `share_plus`. Bunlar aktif ama yalnizca belirli akislar icin gerekli. Kanit: `apps/mobile_flutter/pubspec.yaml`, `apps/mobile_flutter/lib/features/embed/ui/embed_viewer_page.dart`, `apps/mobile_flutter/lib/features/menus/ui/menu_ocr_flow.dart`
- En buyuk release blocker artik signing kodu degil, gercek `android/key.properties` ve keystore secret'larinin release ortamina baglanmasi. Kod tarafi debug signing'den cikartildi ve `assembleRelease` bu eksiklikte bilincli olarak fail ediyor. Kanit: `apps/mobile_flutter/android/app/build.gradle.kts`, `apps/mobile_flutter/android/key.properties.example`
- Build flavor / stage / prod ayrimi net degil. Repo icinde `productFlavors`, `fastlane`, `codemagic`, `appdistribution` veya paralel mobil release zinciri kaniti yok. Kanit: `apps/mobile_flutter/package.json`, `apps/mobile_flutter/android/app/build.gradle.kts`
- Test paketi var. Router tabanli smoke kapsamı genisletildi, deep-link/QR route smoke gercek Android cihazda dogrulandi, devtools payload simulator smoke eklendi, Android native debug push bridge hazirlandi ve `flutter run` + `android_debug_push.ps1` ile gercek cihazda Flutter/router katmanina kadar dogrulandi; true FCM transport/iOS push tap e2e kapsami halen sinirli. Kanit: `apps/mobile_flutter/integration_test/golden_paths_integration_test.dart`, `apps/mobile_flutter/integration_test/live_write_smoke_integration_test.dart`, `apps/mobile_flutter/lib/features/devtools/ui/developer_tools_page.dart`, `apps/mobile_flutter/lib/features/notifications/domain/push_notification_service.dart`, `apps/mobile_flutter/lib/features/notifications/domain/android_debug_push_bridge.dart`, `apps/mobile_flutter/android/app/src/main/kotlin/com/yeedoy/app/MainActivity.kt`, `apps/mobile_flutter/tool/android_debug_push.ps1`, `apps/mobile_flutter/test/core/linking/yeedoy_route_resolver_test.dart`, `apps/mobile_flutter/test/features/notifications/domain/notification_target_path_resolver_test.dart`
- Mobile dokumanlari olusturuldu/guncellendi; bundan sonra drift riski release checklist + test strategy + setup dokumanlarinin birlikte guncel tutulmasina bagli. Kanit: `docs/mobile_release_checklist.md`, `docs/mobile_test_strategy.md`, `docs/setup.md`
...
1. Discovery
   - Giris genelde `'/discover'` veya shell icindeki home yuzeyi.
   - Kanit: `apps/mobile_flutter/lib/features/discovery/ui/discovery_page.dart`
2. Business
   - Discovery sonucu veya deep link ile `'/b/:id'`.
   - Kanit: `apps/mobile_flutter/lib/app/router.dart`, `apps/mobile_flutter/lib/features/business/ui/business_page.dart`
3. Menu
   - `'/b/:id/menu/:menuId'`, `'/menu/:menuId'`, `'/b/:id/menu-item/:itemId'`.
   - Kanit: `apps/mobile_flutter/lib/app/router.dart`, `apps/mobile_flutter/lib/features/menus/ui/menu_page.dart`, `apps/mobile_flutter/lib/features/menus/ui/menu_item_page.dart`
4. Item
   - Fiyat, photo, varyant, history, verify, report akislari item seviyesinde.
   - Kanit: `apps/mobile_flutter/lib/features/menus/ui/menu_item_page.dart`
5. Review / Vote / Contribute
   - Review yazma, helpful vote, report, QR scan, receipt/menu OCR, gecici upload.
   - Kanit: `apps/mobile_flutter/lib/features/reviews/ui/review_create_page.dart`, `apps/mobile_flutter/lib/features/reviews/data/reviews_repository.dart`, `apps/mobile_flutter/lib/features/business/data/report_repository.dart`, `apps/mobile_flutter/lib/features/contribute/ui/contribute_entry.dart`, `apps/mobile_flutter/lib/features/menus/ui/menu_ocr_flow.dart`
6. Profile
   - Profil, achievements, stats, missions, feed, claim/suggestion history, inbox.
   - Kanit: `apps/mobile_flutter/lib/features/profile/ui/profile_page.dart`, `apps/mobile_flutter/lib/features/notifications/ui/inbox_page.dart`
...
- `apps/mobile_flutter/lib/features/business/*`
- `apps/mobile_flutter/lib/features/menus/*`
- `apps/mobile_flutter/lib/features/reviews/*`
- `apps/mobile_flutter/lib/features/favorites/*`
- `apps/mobile_flutter/lib/features/profile/*`
- `apps/mobile_flutter/lib/features/contribute/*`
...
Kanit:
- `apps/mobile_flutter/lib/features/smart_feed/*`
- `apps/mobile_flutter/lib/features/taste_twin/*`
- `apps/mobile_flutter/lib/features/group_requests/*`
- `apps/mobile_flutter/lib/features/top_businesses/*`
- `apps/mobile_flutter/lib/features/gourmets/*`
- `apps/mobile_flutter/lib/features/heroes/*`
- `apps/mobile_flutter/lib/features/budget_combos/*`
- `apps/mobile_flutter/lib/features/price_alerts/*`
- `apps/mobile_flutter/lib/features/suspended_meals/*`
- `apps/mobile_flutter/lib/features/embed/*`
- `apps/mobile_flutter/lib/features/ads/*`
- `apps/mobile_flutter/lib/features/monetization/*`
...
- Shell: `apps/mobile_flutter/lib/app/app_shell.dart`
...
| `budget_combos` | butce odakli menu onerileri | `budget_combo_results_page.dart` | FutureProvider | `budget_combos_repository.dart` | aktif |
| `business` | business detail, amenities, crowd, reports | `business_page.dart` | buyuk StatefulWidget + Riverpod | `business_detail_repository.dart`, `report_repository.dart`, digerleri | en agir modullerden biri |
...
| `contribute` | QR scan, route parse, upload girisi | `contribute_entry.dart` | widget state | `temp_uploads_repository.dart` | aktif |
| `devtools` | dahili debug ayarlari | `developer_tools_page.dart` | widget state + prefs | core prefs/config | dev/debug |
| `discovery` | ana kesif, search, sponsored, campaigns, city filters | `discovery_page.dart` | buyuk StatefulWidget + Riverpod notifier | `discovery_repository.dart`, `search_repository.dart` | en agir modul |
| `embed` | external embed gosterimi | `embed_viewer_page.dart` | StatefulWidget | `embed_repository.dart` | webview/youtube agir |
| `favorites` | favori listeleri ve paylasim | `favorites_page.dart` | widget state + Riverpod | `favorites_repository.dart`, `collection_share_repository.dart`, `collection_social_repository.dart` | aktif |
| `gourmets` | gourmet kesif, following, feed | `gourmets_page.dart`, `following_page.dart`, `feed_page.dart` | Riverpod | `gourmet_repository.dart`, `feed_repository.dart`, `follow_repository.dart` | V2 |
| `group_requests` | grup talebi ve teklif akislari | `my_group_requests_page.dart`, `group_request_wizard_page.dart`, `group_request_detail_page.dart` | Riverpod + widget state | `group_requests_repository.dart` | V2 |
| `heroes` | topluluk/hero leaderboard | `heroes_page.dart` | Riverpod | `hero_repository.dart` | aktif |
| `legal` | policy/legal surface | `legal_page.dart` | Stateless | config + local data | aktif |
| `menus` | menu, item, OCR, share, price verify, photo | `menu_page.dart`, `menu_item_page.dart`, `public_menu_share_page.dart` | AsyncNotifier + widget state | `menu_repository.dart`, `core/media/*`, `food_catalog_repository.dart` | en kritik veri alani |
...
| `notifications` | inbox + device token kaydi | `inbox_page.dart` | Riverpod `AsyncNotifier` | `inbox_repository.dart`, `push_notification_service.dart` | aktif |
| `onboarding` | ilk kullanici akisi | `onboarding_page.dart` | widget state | local prefs / location | aktif |
| `perks` | aktif kampanya/perk listesi | business icine gomulu | FutureProvider | `perk_repository.dart` | aktif ama kucuk |
| `price_alerts` | fiyat alarmi | sheet/tab icinden | Riverpod | `price_alerts_repository.dart` | aktif |
| `profile` | profil, stats, achievements, settings | `profile_page.dart`, `profile_settings_page.dart` | Riverpod + widget state | `profile_repository.dart`, `diet_profile_repository.dart` | aktif |
| `reviews` | review list/create/helpful vote | `business_reviews_page.dart`, `review_create_page.dart` | Riverpod | `reviews_repository.dart` | aktif |
| `shared` | ortak UI kit / widgets / visuals | gomulu | reusable | yok | cift tarafli kullanim yuksek |
| `smart_feed` | personalize feed | `smart_feed_page.dart` | Riverpod | `smart_feed_repository.dart` | V2 |
| `splash` | acilis ekrani | `splash_page.dart` | Stateless | yok | aktif |
| `suggestions` | business suggestion / benim onerilerim | `suggest_business_page.dart`, `my_suggestions_page.dart` | widget state + Riverpod | `suggestions_repository.dart` | aktif |
| `suspended_meals` | askida yemek claim takibi | `my_suspended_claims_page.dart` | Riverpod | `my_suspended_claim_repository.dart` | V2 |
| `taste_twin` | taste graph/match | `taste_twin_page.dart` | Riverpod | `taste_twin_repository.dart` | V2 |
| `top_businesses` | donemsel en iyi isletmeler | `top_businesses_page.dart` | Riverpod | `top_businesses_repository.dart` | aktif |

### 3.3 Bos / Stub / Olu Kalan Parcalar

| Path | Durum | Neden |
|---|---|---|
| `apps/mobile_flutter/lib/app/splash/` | bos klasor | klasor var, icinde dosya yok |
| `apps/mobile_flutter/integration_test/*` | aktif smoke | `golden_paths` route zinciri + runtime gate + deep-link/QR route smoke + devtools payload simulator smoke ve `live_write_smoke` opt-in backend write smoke mevcut; Android native debug push bridge eklendi, true FCM transport/iOS e2e genisletmesi kaldi |

### 3.4 Kaldirilmali Listesi

Bu bolum "hemen sil" degil, audit karari olarak okunmali:

- `apps/mobile_flutter/lib/app/splash/`
...
- `upsert_user_location_prefs_v1` inferans: `user_location_repository.dart`
...
- `get_sponsored_businesses_v1` inferansi `discovery_repository.dart` icinden
- `set_favorite_v2`
- `toggle_favorite_v1`
- `list_favorites_v1`
...
- In-memory TTL cache var. Kanit: `apps/mobile_flutter/lib/core/cache/ttl_memory_cache.dart`
- `SharedPreferences` tabanli disk cache var. Kanit: `apps/mobile_flutter/lib/core/storage/offline_cache_prefs.dart`
- bucket bazli SQLite local DB var. Kanit: `apps/mobile_flutter/lib/core/storage/local_db/sqflite_local_db_store.dart`
- Saklanan ana veri tipleri:
  ...
  - favorites snapshot
  - categories snapshot
- Offline queue verify / suggestion + submission draft katmanlariyla genisledi. Kanit: `apps/mobile_flutter/lib/features/menus/data/offline_verify_queue.dart`, `apps/mobile_flutter/lib/core/storage/offline_submission_queue.dart`
...
- Discovery sayfasi `CustomScrollView` / `SliverList` kullaniyor. Kanit: `apps/mobile_flutter/lib/features/discovery/ui/discovery_page.dart`
- Ozel image cache manager var. Kanit: `apps/mobile_flutter/lib/core/media/app_image_cache_manager.dart`
- `precacheImageUrls` kullanim izleri var. Kanit: `apps/mobile_flutter/lib/features/menus/ui/menu_item_page.dart`
- Firebase Performance ve app telemetry entegre. Kanit: `apps/mobile_flutter/lib/core/perf/firebase_perf_trace.dart`, `apps/mobile_flutter/lib/core/monitoring/app_telemetry.dart`
...
- Search/discovery/business arasi request coalescing ve merkezi cache katmani yok.
...
1. `discovery_page.dart` ekranini en az 4 alt surface'e bol.
   - Etki: HIGH
   - Dosya: `apps/mobile_flutter/lib/features/discovery/ui/discovery_page.dart`
2. `business_page.dart` icindeki sections'i ayri ConsumerWidget siniflarina parcala; reviews, perks, crowd ve report alanlarini lazy render et.
   - Etki: HIGH
   - Dosya: `apps/mobile_flutter/lib/features/business/ui/business_page.dart`
3. `menu_item_page.dart` icin tab/section bazli widget ayrisma yap; photo/history/value score kartlarini ayri provider ile izole et.
   - Etki: HIGH
   - Dosya: `apps/mobile_flutter/lib/features/menus/ui/menu_item_page.dart`
4. `embed_viewer_page.dart` icinde provider secimini sertlestir; YouTube ve WebView controller init'ini buton/URL turune gore gec baslat.
   ...
   - Etki: MED
   - Dosya: `apps/mobile_flutter/lib/features/embed/ui/embed_viewer_page.dart`
   - Ek dosyalar:
     - `apps/mobile_flutter/lib/features/embed/ui/providers/embed_provider_youtube.dart`
     - `apps/mobile_flutter/lib/features/embed/ui/providers/embed_provider_webview.dart`
5. `menu_ocr_flow.dart` icinde OCR sonucu parse + upload + match sheet akisini servis katmanina ayir.
   - Etki: MED
   - Dosya: `apps/mobile_flutter/lib/features/menus/ui/menu_ocr_flow.dart`
6. `AppImageCacheManager` sabit `400` obje limitini device class veya disk butcesine gore ayarlanabilir yap.
   - Etki: MED
   - Dosya: `apps/mobile_flutter/lib/core/media/app_image_cache_manager.dart`
7. Business ve menu detaylarinda image prefetch'i "visible range" ile sinirla; su an sayfa icinde agresif prefetch riski var.
   - Etki: MED
   - Dosya: `apps/mobile_flutter/lib/features/menus/ui/menu_item_page.dart`
8. Search/discovery icin merkezi request cancellation ve stale request ignore katmani ekle.
   - Etki: MED
   - Dosya: `apps/mobile_flutter/lib/features/discovery/data/search_repository.dart`, `apps/mobile_flutter/lib/features/discovery/ui/discovery_page.dart`
9. Sponsored / ads / native ad yuzeyleri ilk layout'u bloklamamali; loading placeholder standardini merkezi hale getir.
   - Etki: MED
   - Dosya: `apps/mobile_flutter/lib/features/ads/data/native_ad_controller.dart`, `apps/mobile_flutter/lib/features/discovery/ui/discovery_page.dart`
10. `flutter_screenutil` ile token sistemi disindaki `const EdgeInsets/SizedBox/TextStyle` kullanimlarini azalt.
   - Etki: MED
   - Dosya: `apps/mobile_flutter/lib/app/app_shell.dart`, `apps/mobile_flutter/lib/features/chains/ui/chain_page.dart`, `apps/mobile_flutter/lib/features/business/ui/business_page.dart`
11. `google_fonts` sadece splash icin kullaniliyorsa, asset font ile tek kaynaga in.
   - Etki: LOW
   - Dosya: `apps/mobile_flutter/lib/features/splash/ui/splash_page.dart`, `apps/mobile_flutter/assets/fonts/*`
12. `analytics_repository.logEvent()` icin background queue veya batch mekanizmasi dusun; fire-and-forget tamamen sessiz kayip olusturuyor.
   - Etki: LOW
   - Dosya: `apps/mobile_flutter/lib/core/analytics/analytics_repository.dart`

## 7) Guvenlik / Gizlilik

### 7.1 Mevcut Pozitifler

- Supabase session secure storage ile tutuluyor. Kanit: `apps/mobile_flutter/lib/core/security/secure_local_storage.dart`
- Critical action guard var. Kanit: `apps/mobile_flutter/lib/core/security/critical_action_guard.dart`
- Edge rate limit guard var. Kanit: `apps/mobile_flutter/lib/core/security/edge_rate_limit_guard.dart`
- Write gatekeeper client var. Kanit: `apps/mobile_flutter/lib/core/security/write_gatekeeper_client.dart`
- Crashlytics ve Performance aktif. Kanit: `apps/mobile_flutter/lib/main.dart`, `apps/mobile_flutter/lib/core/perf/firebase_perf_trace.dart`

### 7.2 Riskler

- Android release artik debug signing'e dusmuyor; fakat prod secret/keystore baglantisi kurulmadan release alinmaz. `flutter build apk --release -t lib/main_mobile.dart` bu ortamda tam da bu nedenle fail etti.
  - Risk: HIGH
  - Kanit: `apps/mobile_flutter/android/app/build.gradle.kts`, `apps/mobile_flutter/android/key.properties.example`
...
- `analytics_repository.logEvent()` hatalari yutuyor; audit/telemetry kaybi sessiz olabilir.
  - Risk: MED
  - Kanit: `apps/mobile_flutter/lib/core/analytics/analytics_repository.dart`
- Push service platform kararinda `kIsWeb ? 'web'` branch'i duruyor; product scope mobil oldugu icin gereksiz genislik ve yanlis kayit ihtimali yaratabilir.
  - Risk: LOW
  - Kanit: `apps/mobile_flutter/lib/features/notifications/domain/push_notification_service.dart`
- OCR / upload / report gibi write akislarinda PII veya raw error metinleri merkezi scrub politikasina bagli degil.
  - Risk: MED
  - Kanit: `apps/mobile_flutter/lib/core/errors/app_error_mapper.dart`, `apps/mobile_flutter/lib/core/media/media_upload_client_io.dart`
...
- Tema temeli guclu.
- `ScreenUtilInit` ve token extension kullanimi mevcut. Kanit: `apps/mobile_flutter/lib/app/app.dart`, `apps/mobile_flutter/lib/app/theme/app_theme.dart`
...
- `apps/mobile_flutter/lib/features/chains/ui/chain_page.dart`
- `apps/mobile_flutter/lib/features/business/ui/business_page.dart`
- `apps/mobile_flutter/lib/features/ads/ui/native_ad_card.dart`
- `apps/mobile_flutter/lib/app/router.dart`
...
- Unit:
  - `apps/mobile_flutter/test/core/cache/ttl_memory_cache_test.dart`
  - `apps/mobile_flutter/test/core/quality/release_gate_test.dart`
  - `apps/mobile_flutter/test/core/growth/ab_experiments_test.dart`
- Contract:
  - `apps/mobile_flutter/test/core/contracts/discovery_api_contract_test.dart`
- UI quality:
  - `apps/mobile_flutter/test/ui/design_system_quality_gates_test.dart`
- Golden:
  - `apps/mobile_flutter/test/ui/golden/basic_surfaces_golden_test.dart`
- Integration:
...
- Golden test env flag ile kapali. Kanit: `apps/mobile_flutter/test/ui/golden/basic_surfaces_golden_test.dart`
- Integration smoke gercek `YeedoyApp` router'ini ayaga kaldiriyor ve route zinciri + report sheet + review-login redirect kapsiyor. Kanit: `apps/mobile_flutter/integration_test/golden_paths_integration_test.dart`
- Canli backend write smoke icin opt-in test eklendi: `review/report/menu price suggestion/favorites add-remove` ve `unauth fail-path`. Kanit: `apps/mobile_flutter/integration_test/live_write_smoke_integration_test.dart`
...
- `README` ve `package.json` Android APK release akisini one cikarmaya devam ediyor.
  - Kanit: `apps/mobile_flutter/README.md`, `apps/mobile_flutter/package.json`
- Repo-root mobile CI/readiness workflow'lari ve iOS readiness dokumani artik var:
  ...
  - `apps/mobile_flutter/tool/ios_readiness_check.dart`
...
- HIGH: Android release keystore secret'lari ortama baglanmamis olabilir
  ...
  - `apps/mobile_flutter/android/key.properties.example`
- HIGH: Gercek cihazda true FCM transport'tan gelen push tap payload dahil tam e2e smoke henuz yok
  - `apps/mobile_flutter/integration_test/golden_paths_integration_test.dart`
  - `apps/mobile_flutter/integration_test/live_write_smoke_integration_test.dart`
- MED: Stage/prod flavor yok
  - `apps/mobile_flutter/package.json`, `apps/mobile_flutter/android/app/build.gradle.kts`
...
| `docs/data-model.md` | mobile'in gercek RPC listesi ve write/read ayrimi eklenmeli |
...
| `docs/runbook.md` | mobile release smoke adimlari panel/web ayri sekilde eklenmeli |
| `docs/devtools.md` | mobile devtools ekraninin scope'u ve prod disi kural netlestirilmeli |
| `docs/mobile_test_strategy.md` | integration/golden/contract/load test kapsami guncel tutulmali |
...
|---|---|---|---|
| Gercek Android keystore secret'larini bagla ve `assembleRelease` dogrulamasini gec | HIGH | M | `apps/mobile_flutter/android/app/build.gradle.kts`, `apps/mobile_flutter/android/key.properties` |
| Gercek mobile smoke test yaz: splash -> login -> discovery -> business -> menu -> review/report | HIGH | M | `apps/mobile_flutter/integration_test/*` |
| `mobile_release_checklist.md` olustur | HIGH | S | `docs/mobile_release_checklist.md` |
| `mobile_supabase_contracts.md` olustur | HIGH | M | `docs/mobile_supabase_contracts.md` |
| Gercek business/menu/review write smoke'unu emulator/device uzerinde genislet | HIGH | M | `apps/mobile_flutter/integration_test/golden_paths_integration_test.dart` |
...
- Smoke:
  ...
  - `live_write_smoke_integration_test.dart` eklendi (opt-in): gercek backend review/report/menu/favorites write + unauth fail-path.
- Dokuman:
  - `docs/mobile_release_checklist.md`, `docs/mobile_test_strategy.md`, `docs/setup.md`, `apps/mobile_flutter/README.md` ilgili akislara gore guncellendi.

### P1 (2-4 hafta)

| Aksiyon | Etki | Zorluk | Ilgili dosya/klasor |
|---|---|---|---|
| `discovery_page.dart` ekranini alt surface'lere bol | HIGH | L | `apps/mobile_flutter/lib/features/discovery/ui/discovery_page.dart` |
| `business_page.dart` ekranini section bazli ayir | HIGH | L | `apps/mobile_flutter/lib/features/business/ui/business_page.dart` |
| `menu_item_page.dart` ekranini tab/section bazli ayir | HIGH | L | `apps/mobile_flutter/lib/features/menus/ui/menu_item_page.dart` |
| Merkezi request caching / invalidation katmani kur | HIGH | M | `apps/mobile_flutter/lib/core/cache/*`, `features/*/data/*` |
| Offline queue'yu report/review/suggestion draft icin genislet | HIGH | M | `apps/mobile_flutter/lib/features/*/data/*` |
...
| Push token lifecycle ve platform kaydini daralt | MED | S | `apps/mobile_flutter/lib/features/notifications/domain/push_notification_service.dart` |
| `analytics_repository` icin daha guvenilir retry/batch tasarla | MED | M | `apps/mobile_flutter/lib/core/analytics/analytics_repository.dart` |
| Token-disi design usage cleanup backlog'unu baslat | MED | M | `apps/mobile_flutter/lib/app/app_shell.dart`, `apps/mobile_flutter/lib/features/business/ui/business_page.dart` |
...
- Merkezi request cache/invalidation adimi tamamlandi:
  ...
  - discovery/search/menu repository'leri ortak cache scope mekanizmasina tasindi
  - `search_repository.clearCache()` + `discovery_search_notifier.refresh()` ile pull-to-refresh cache bypass akisi eklendi
  - `discovery_repository.getBusiness()` TTL cache + explicit invalidate destegi aldi
  - `menu_repository.clearReadCache()` eklendi ve business refresh akisina baglandi
- Discovery ekran ayristirma adimi baslatildi:
  - `discovery_page.dart` icindeki campaign/map/insight surface'leri ayri dosyalara tasindi:
    - `features/discovery/ui/surfaces/discovery_campaigns_tab.dart`
    - `features/discovery/ui/surfaces/discovery_map_surface.dart`
    - `features/discovery/ui/surfaces/discovery_insight_sections.dart`
...
- Business ekran ayristirma adimi baslatildi:
  - `business_page.dart` icindeki header/actions/trust/hours/menus/crowd/reviews/perks/footer section siniflari ayri dosyaya tasindi:
...
- Analytics repository guvenilirlik adimi tamamlandi:
  - `core/analytics/analytics_repository.dart` icine in-memory queue + retry/backoff + batch flush eklendi.
  - `logEvent` API'si korunarak basarisiz event'lerin yeniden gonderim denemesi merkezi hale getirildi.
  - `test/core/analytics/analytics_repository_test.dart` ile kuyruklama, drop ve meta birlestirme davranislari test altina alindi.
- Offline queue genisletme adimi tamamlandi:
  ...
  - report/review/business suggestion write'lari offline durumda queue'ya alinacak sekilde guncellendi:
    - `features/business/data/report_repository.dart`
    - `features/reviews/data/reviews_repository.dart`
    - `features/suggestions/data/suggestions_repository.dart`
  - UI tarafinda offline queue sonucu icin geri bildirim eklendi:
    ...
    - `features/suggestions/ui/suggest_business_page.dart`
...
- Token-disi design cleanup backlog'u baslatildi:
  - `app/app_shell.dart` icinde app bar title style tema tipografisine baglandi.
  ...
  - `features/chains/ui/chain_page.dart` icinde spacing/typography kullanimlari `AppTokens` + theme extension uzerine tasindi.
...
| Offline queue'yu report/review/suggestion draft icin genislet | Tamam |
...
| `analytics_repository` icin daha guvenilir retry/batch tasarla | Tamam |
...
| Aksiyon | Etki | Zorluk | Ilgili dosya/klasor |
|---|---|---|---|
| Lokal DB tabanli gercek offline mode degerlendirmesi yap | HIGH | L | `apps/mobile_flutter/lib/core/storage/*` |
| Embed akislarini URL-provider bazli daha hafif hale getir | MED | M | `apps/mobile_flutter/lib/features/embed/ui/embed_viewer_page.dart` |
| OCR/upload akislarini servis ve queue mimarisine tasi | MED | L | `apps/mobile_flutter/lib/features/menus/ui/menu_ocr_flow.dart` |
| Native ads ve sponsorlu kesif icin performans/biz logic ayristirmasi yap | MED | M | `apps/mobile_flutter/lib/features/ads/*`, `discovery/*` |
| Search / discovery telemetry dashboard sozlesmesini yaz | MED | M | `apps/mobile_flutter/lib/core/monitoring/*` |
| Image cache policy'yi cihaz profiline gore optimize et | LOW | M | `apps/mobile_flutter/lib/core/media/app_image_cache_manager.dart` |
| `google_fonts` / asset font overlap'ini temizle | LOW | S | `apps/mobile_flutter/lib/features/splash/ui/splash_page.dart` |
...
- Embed akisi URL-provider bazli alt surface'lere ayrildi:
  - `features/embed/ui/providers/embed_provider_youtube.dart`
  - `features/embed/ui/providers/embed_provider_webview.dart`
...
- Embed hardening adimi tamamlandi:
  - invalid URL ve invalid YouTube source icin widget testleri eklendi:
    - `test/features/embed/ui/embed_viewer_page_test.dart`
  - invalid URL fallback integration smoke eklendi:
    - `integration_test/embed_smoke_integration_test.dart`
...
- Native ads + sponsored discovery ayristirma adimi baslatildi:
  - ad insertion kurali `features/discovery/domain/discovery_feed_composer.dart` icine tasindi
  ...
  - testler eklendi:
    - `test/features/discovery/domain/discovery_feed_composer_test.dart`
    - `test/features/monetization/domain/sponsored_businesses_provider_test.dart`
- Search/discovery telemetry dashboard sozlesmesi yazildi:
  - `docs/mobile_discovery_telemetry_contract.md` olusturuldu
...
- Lokal DB tabanli offline mode degerlendirme + skeleton eklendi:
  - `docs/mobile_local_db_offline_plan.md` olusturuldu (faz/migration/risk plani)
  - core abstraction + memory adapter eklendi:
    - `core/storage/local_db/local_db_models.dart`
    - `core/storage/local_db/local_db_store.dart`
    - `core/storage/local_db/memory_local_db_store.dart`
    - `core/storage/local_db/local_db_provider.dart`
  - `test/core/storage/local_db/memory_local_db_store_test.dart` ile temel davranislar test altina alindi
- Lokal DB / replay adimi ilerletildi:
  - `core/storage/local_db/shared_prefs_local_db_store.dart` ile disk-backed gecis adapter'i devreye alindi
...
- Lokal DB / replay adimi sertlestirildi:
  - `core/storage/local_db/sqflite_local_db_store.dart` ile SQLite tabanli bucket tablolar devreye alindi
  - `core/storage/local_db/local_db_provider.dart` Android/iOS tarafinda SQLite, web/init-failure durumunda shared-prefs fallback kullanacak sekilde guncellendi
...
- Retry/backoff policy action bazli hale getirildi:
  ...
  - `docs/runbook.md` icinde operator recovery akisi yazildi
- Deneysel/kismi akislarda request-cache hardening adimi tamamlandi:
  - repository rollout: `smart_feed`, `taste_twin`, `gourmets`, `group_requests`, `suspended_meals`, `budget_combos`
  - force refresh baglantilari: `smart_feed_page`, `taste_twin_page`, `gourmets/following/feed`, `group_requests`
...
- Push tap route handling adimi genisletildi:
  ...
  - `inbox_repository` ve push tap ayni resolver'i kullanir: `lib/features/notifications/domain/notification_target_path_resolver.dart`
...
- `google_fonts` / asset font overlap temizlendi:
  ...
  - uygulama tema fontu `app/theme/app_theme.dart` icinde `fontFamily: 'Sora'` olarak tek kaynaga alindi
