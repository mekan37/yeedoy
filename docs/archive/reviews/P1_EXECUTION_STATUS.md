...
|---|---|
| **Status** | ✅ Done |
| **Verified in** | `apps/panel_flutter_web/lib/features/auth/ui/login_page.dart`, `apps/web_next/app/auth/panel-handoff/route.ts`, `apps/web_next/app/login/page.tsx` |

**What exists:**
- Panel builds QR URL via `buildQrLoginUrl()` → redirects to `/login?redirect=/qr/{businessId}?lang=...&theme=...`
- After login, `POST /auth/panel-handoff` receives `access_token`, `refresh_token`, `business_id`, `lang`, `theme`
...
|---|---|
| **Status** | ✅ Done |
| **Verified in** | `apps/web_next/app/qr/[businessId]/page.tsx`, `apps/web_next/src/lib/qr-access.ts`, `apps/web_next/app/forbidden/route.ts` |
...
|---|---|
| **Status** | ✅ Done |
| **Verified in** | `apps/mobile_flutter/integration_test/live_write_smoke_integration_test.dart`, `apps/web_next/e2e/live-smoke.helpers.ts`, `apps/web_next/e2e/public-menu-live.spec.ts`, `apps/panel_flutter_web/e2e/panel-smoke.spec.cjs` |
...
- Panel: Playwright smoke already covers owner/admin authenticated flows in addition to public/legal routes
...
|---|---|
| **Status** | ✅ Done |
| **Verified in** | `apps/panel_flutter_web/lib/features/admin/data/admin_queue_repository.dart`, `apps/panel_flutter_web/lib/features/admin/data/admin_reports_repository.dart`, `apps/panel_flutter_web/lib/features/admin/data/admin_claims_repository.dart` |
...
|---|---|
| **Status** | ✅ Done |
| **Verified in** | `apps/panel_flutter_web/lib/features/admin/ui/admin_observability_page.dart` (57.6KB) |

**What exists (comprehensive):**
- **Offline mutation monitoring:** Lists offline mutation outcomes across 6h/24h/72h windows; alert settings editor (min signal, retry/drop/auth/rate-limit thresholds); load/save to admin repository
...
|---|---|
| **Status** | ✅ Done |
| **Verified in** | `apps/panel_flutter_web/lib/core/security/app_role_providers.dart`, `supabase/migrations/20260303142038_owner_team_rbac_and_impersonation.sql`, legacy migration files |
...
| **Verified in** | Grep across `apps/panel_flutter_web/lib/features/` |
...
| P1-4 | Admin reports/claims `total_count` pagination | ✅ Done | Medium |
...
| `docs/roadmap.md` | P1-1 and P1-2 listed as open | Both are already resolved — should be closed |

See `docs/drift_report.md` for running diff tracking.
