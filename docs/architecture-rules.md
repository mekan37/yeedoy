...
Yasak:
- Public SEO menu render
- Public QR landing sayfasini panel icinde yeniden yazmak

### `apps/web_next`

Izinli:
- Public menu render
- QR Studio, branding upload, panel handoff
...
Kurallar:
- Yeni Supabase sorgusu repository'ye gider.
- Controller'lar ince kalir; UI state'i repository cagirir.
...
- `app/**/page.tsx`: route, metadata, redirect/notFound
- `app/api/**/route.ts`: mutation/boundary
- `src/lib/db/*`: read model
- `src/ui/sections/*`: interaktif client surface

Kurallar:
- Public data okuma `src/lib/db/menu-read.ts` ve yakin helper'larda tutulur.
...
- Flutter: `repository -> provider/controller -> page/widget`
- Panel: `PanelShell` etrafinda tek owner/admin iskeleti
- Web: `page.tsx -> src/lib -> client section` ayrimi
